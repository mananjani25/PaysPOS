...
fun addCreditCardBreakDownData(
    ...
    var pOne = creditCardBreakdown.key + repeat(
        " ",
        28 - creditCardBreakdown.key.length
    ) + if (creditCardBreakdown.key.contains("Refund")) "-" + MethodUtils.roundOffAmount(creditCardBreakdown.tips) else MethodUtils.roundOffAmount(creditCardBreakdown.tips)
    var lastPart = 48 - pOne.length
...
fun addOrdersForKitchenOnlineOrderKiosk(
    builder: Builder,
...
fun addOrdersForKitchenOnlineOrderU220Kiosk(
    builder: Printer,
    list: List<KioskOrderResponse.Data.OrderItems>,
    fontSizeH: Int = 1,
    fontSizeW: Int = 1,
...
fun addOrdersForKitchenOnlineOrderSunmiInnerKiosk(
...
fun addOrdersForKitchenOnlineOrderSunmiInnerKioskNew(
    ...
    for (i in 0 until list.size) {
        printerCat?.forEach {
            if (it.id == list[i].categoryId && it.printerEnable && it.categoryActive) {
                val obj = list.get(i)
                PrintSunmiUtils.normalTextLargeNew(obj.quantity.toString() + " " + obj.itemName?.uppercase())

                if (obj.orderItemModifiers.isNotEmpty()) {
                    for (j in 0 until obj.orderItemModifiers.size) {
                        val modifierObj = obj.orderItemModifiers.get(j)
//                         Modifiers are not coming from server, Once Urmit send's it, then we will uncomment the below code
                        PrintSunmiUtils.normalTextLargeNew(
                            "  " + if (modifierObj.modifierQuantity == 1) {
                                "   "
                            } else {
                                "" + modifierObj.modifierQuantity + "x "
...
fun addReprintOrdersForStarKitchenKiosk(
    list: List<KioskOrderResponse.Data.OrderItems>,
    printerCat: ArrayList<PrinterResponse.Data.PrinterCategories?>? = null
): String {
