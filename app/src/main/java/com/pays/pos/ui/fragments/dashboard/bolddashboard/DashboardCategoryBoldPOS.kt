...
import com.pays.pos.data.remote.Constants.OPEN_ORDER_ITEMS
...
//            }
    ...
    override fun onCreateView(
        ...
        prefProvider?.setValue(
            Constants.SUNMI_FRAMEWORK_VERSION,
            SystemProperties.get("ro.version.sunmi_versionname")
        )
        /*--------------------- FOR CUSTOMER RECEIPT MODIFICATION--------------------*/
                            ...
                            if (it.data.isNotEmpty()) {
                                try {
                                    if (it.data.size < pageSize.toInt()) {
                                        var status = viewModel.addCustomersList(
                                            currentpage,
                                            it.data,
                                            lastCall = true
                                        )
                                        ...
                                        var status =
                                            viewModel.addCustomersList(currentpage, it.data)
                                    }
...
//        System.gc()
        ...
        */
        ...
        if (prefProvider.getValueboolean(SPLIT_ENABLE, false) || prefProvider.getValueboolean(
                IS_PAX_PAYMENT_FAILED,
                false
            )
        ) {
            ...
            event.getContentIfNotHandled()?.let {
                if (it) {
                    AlertUtils.showCustomAlert(
                        requireContext(),
                        "${getString(R.string.item_quantity_cannot_exceed)} ${MAX_ITEM_QUANTITY}"
                    )
                }
            }
        }
    ...
    private fun checkDineInEditOrder() {
        if (arguments?.getBoolean("is_dine_in_edit") == true) {
            ...
            if (dineInList?.isNotEmpty() == true) {
                ...
                cartList[0].discountSelectdValue =
                    arguments?.getDouble("discountSelectdValue") ?: 0.0
    ...
    private fun initKitchenPrinter(
        ...
        if (data.name.startsWith("CloudPrint",true) && data.printer_type == WIFI){
            ...
            if (prefProvider.getValueboolean(ORDER_NUMBER_STARTING_FROM_ONE, false)) {
                appendText("OrderID:${createOrderResponse.data?.order?.custom_order_id}")
            } else {
                appendText(":${createOrderResponse.data?.order?.id}")
            }

            lineFeed(2)
            appendText(""+createOrderResponse.data?.order?.orderTypeName)
            lineFeed(2)
//        setCharacterSize(1,1)
            setAlignment(0)
            appendText("Employee:${createOrderResponse.data?.order?.employee.name}")
            lineFeed(2)
            ...
            var orderItems = createOrderResponse.data?.order?.orderItems ?: arrayListOf()
            ...
            if(createOrderResponse.data?.order?.customer != null){
                ...
                if (createOrderResponse.data?.order?.customer?.firstName != null && createOrderResponse.data?.order?.customer?.lastName != null){
                    appendText(createOrderResponse.data?.order?.customer?.firstName+" "+createOrderResponse.data?.order?.customer?.lastName)
                    lineFeed(1)
                }
    ...
    private fun observeSaveOrder() {
        ...
        viewModelPayment.QueueStart.observe(viewLifecycleOwner) { event ->
            event.getContentIfNotHandled()?.let { it ->
                ...
                val model = viewModel.cartModel
                Log.d("DashboardCategoryBoldPOS:", Gson().toJson(model))
 ...
 */
                                    ...
                                    for (oldDataIndex in 0 until oldDataModel.size) {
                                        ...
                                        for (newDataIndex in oldDataIndex until createOrderResponse.data.order.orderItems.size) {
                                            var newData =
                                                createOrderResponse.data.order.orderItems.get(
                                                    newDataIndex
                                                )
                                            if (oldDataIndex == newDataIndex) {
                                                if (oldData.categoryId == newData.categoryId &&
                                                    oldData.completedInKitchen == newData.completedInKitchen &&
                                                    oldData.discountAmount == newData.discountAmount &&
                                                    oldData.discountId == newData.discountId &&
                                                    oldData.discountType == newData.discountType &&
                                                    oldData.employeeId == newData.employeeId &&
                                                    oldData.float == newData.float &&
                                                    oldData.guestIndexForDineIn == newData.guestIndexForDineIn &&
                                                    oldData.id == newData.id &&
                                                    oldData.isPaid == newData.isPaid &&
                                                    oldData.isPrinted == newData.isPrinted &&
                                                    oldData.itemId == newData.itemId &&
                                                    oldData.itemName == newData.itemName &&
                                                    oldData.note == newData.note &&
                                                    oldData.orderItemModifiers == newData.orderItemModifiers &&
                                    ...
                                    if (itemsNotUpdated != createOrderResponse.data.order.orderItems.size) {
                                    ...
                                    if (oldDataModel.size != createOrderResponse.data.order.orderItems.size) {
                                        ...
                                        for (item in createOrderResponse.data.order.orderItems) {
                                            if (item.isItemEdited) {
                                                ...
                                                if(currentItemIds.toList() != oldItemIds.toList()){
                                                    isOrderUpdate = true
                                                    break
                                                }
                                            }
...
//                                        }
    ...
    private fun setupInnerPrinterAttributes(innerPrinterModel: PrinterListModel) {
        ...
        EventBus.getDefault()
            .post(
                MessageEvent(
                    "${Constants.LINE_BREAK_TAB} DashboardCategoryBoldPOS.kt_ setupInnerPrinterAttributes_createPrinter= ${
                        Gson().toJson(
                            createPrinter
                        )
                    }"
    ...
    private fun setCreatePrintersObserver() {
        ...
        runOnUiThread(kotlinx.coroutines.Runnable {
            printerViewModel.disableCompleteTouch.observe(viewLifecycleOwner,
                object : Observer<Event<Boolean>> {
                    override fun onChanged(t: Event<Boolean>?) {
