                                    ...
                                    lineBreak()*/
                                        ...
                                        if (order!!.payments.last().paymentType == "Cash") {
                                            val changeAmt = padLine(
                                                "Change Amount",
                                                "$" + MethodUtils.roundOffAmountString(((paidAmount) - (order!!.payments.last().amount + tipAmount))),
                                                if (customerSettingModel.fonts == LARGE) 23 else 48
                                            ).toString()
                                ...
                                with(printerBuilder) {
                                    ...
                                    if (!oneItemPerReceipt) {
                                        receiptModel?.order?.orderItems?.forEach { item ->
                                            if (!item.isPrinted) { // In case of Single item per receipt, isPrinted variable is maintained
