                ...
                viewModelDashBoard.apply {
                    ...
                    if (paymentTypeForTip.equals("cash", true)) {
                        ...
                        if (!employeeGivenTip && changeAmount < 0.0 || changeAmount > 0.0) {
                            ...
                            if (changeAmount.toString()
                                    .substring(changeAmount.toString().indexOf('.')).length == 2
                            ) {
                            ...
                            val _title =
                                if (changeAmount < 0) "$" + /*Math.abs(changeAmount)*/ changeAmountFormatted
...
//                            dashboardViewModel.setCollectMore(_title)
                    ...
                    } else if (paymentTypeForTip.toLowerCase().equals(
                            "card",
                            true
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            try {
                oneItemPerReceipt =
                    dashboardViewModel.getLabelPrinterSettingsData().oneItemPerReciept
            } catch (e: Exception) {
...
//        ProgressUtils.showProgressDialog(requireActivity())
        ...
        if (isDineIn) {
            ...
            receiptModel?.order?.orderItems =
                receiptModel?.order?.orderItems?.sortedBy { it.itemName.lowercase() }!!
        }
                                                ...
                                                dineInList.get(i).item?.let { item ->
                                                    ...
                                                    val itemDataToPrint =
                                                        padLineCustomerItem(
                                                            ...
                                                            getItemPriceToShow(obj.price * obj.itemQuantity),
                                                            48
...
//                                                                    },
                                                                    ...
                                                                    getModifierItemPriceToShow(
                                                                        modifierObj.price,
                                                                        obj.itemQuantity * modifierObj.modifier_quantity
                                                                    ),
                                                                    48
                                                                ).toString().toByteArray()
...
//                            "-$" + MethodUtils.roundOffAmountString(0.00)
                                    ...
                                    try {
                                        fetchSubTotalFromPreference = prefProvider.getValue(
                                            DINE_IN_SUB_TOTAL_AMOUNT_BEFORE_PAYMENT,
                                            fetchSubTotalFromPreference.toString()
                                        ).toDouble()
                                    } catch (e: Exception) {
...
//        }
        ...
        if (getDineInOrderDetails?.payments?.isNotEmpty() == true) {
            printPayment(
                prefProvider.isOldSunmiFrameworkVersion(),
                printerType = SUNMI_INNER_PRINTER,
                list = getDineInOrderDetails!!.payments
            )
                                            ...
                                            if (order?.customer != null) {
                                                ...
                                                if (customerSettingModel.showCustomerAddress) {
                                                    if (order?.customer?.addresses?.isNotEmpty() == true) {
                                                        ...
                                                        order.customer.addresses.filter { it.typeOfAddress == SHIPPING_ADDRESS }
                                                            .forEach {
...
//                                        printCenter()
                                        ...
                                        if (customerSettingModel.showQrCode) {
                                            LPrint.printQRCode(
                                                outputStream,
                                                order?.digital_receipt_url.toString(),
                                                LPrint.CENTER_ALIGN
                                            )
                                        }
...
//                                            PrintSunmiUtils.headerText("***** UPDATED *****")
                                        ...
                                        if (kitchenSettingModel.showOrderType) {
                                            printCenter(
                                                receiptModel?.order?.orderTypeName.toString(),
                                                isBold = true,
                                                fontSize = FONT_SIZE_5X
                                            )
                                        }
                                        ...
                                        if (receiptModel?.order?.orderType.equals(
                                            ...
                                            printCenter(
                                                receiptModel?.order?.deliveryType.toString(),
                                                isBold = true,
                                                fontSize = FONT_SIZE_5X
                                            )
                                            lineBreak()
                                            ...
                                            if (sunmiFrameworkVersion?.get(0)
                                                ...
                                                if (kitchenSettingModel.showCustomerAddress) {
                                                    if (receiptModel?.order?.orderType?.trim()
                                                            ...
                                                            .toString().lowercase()
                                                        && receiptModel?.order?.deliveryType?.trim()
                                                            .toString()
...
//            presentation.hide()
    ...
    private fun sunmiPrinterInit(ipAddress: String, isAutoPrint: Boolean) {
        ...
        try {
            SunmiPrinterApi.getInstance()
                .setPrinter(SunmiPrinter.SunmiBlueToothPrinter, ipAddress)
        } catch (e: Exception) {
    ...
    private fun sunmiCloudPrintForGiftCard() = try {
        ...
        Log.d(
            TAG,
            "sunmiCloudPrintForGiftCard: giftCardAmount = ${giftCardAmount.toPrecision(2)}"
        )
