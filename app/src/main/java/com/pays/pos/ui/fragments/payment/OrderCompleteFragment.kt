...
@AndroidEntryPoint
class OrderCompleteFragment : Fragment(), View.OnClickListener, StatusChangeEventListener, BatteryStatusChangeEventListener, ICallback {
...
//        observeValorTipFromCustomerDisplay()
        lifecycleScope.launch(Dispatchers.Main) {
            try {
                printOrderIDInStickyPrinter = dashboardViewModel.getLabelPrinterSettingsData().printOrderId
            } catch (e: Exception) {
...
//        3.3.39
        ...
        getCustomerDisplay(requireContext())?.let { display ->
            presentation = CustomDisplay(
                display, requireContext(), viewLifecycleOwner, dashboardViewModel, passcodeViewModel, dineInViewModel
        ...
        findNavController().currentBackStackEntry?.savedStateHandle?.getLiveData<Bundle>("data")?.observe(viewLifecycleOwner) { it ->
        ...
        noCashAdjGlobal = MethodUtils.roundOffAmountDouble(requireArguments().getDouble("noCashAdj"))
    ...
    override fun onResume() {
        ...
        if (this::presentation.isInitialized) {
            ...
            if (prefProvider.getValueboolean(
                    Constants.TIP_ADDED, false
                ) && !dashboardViewModel.customerGivenTipBefore.value!!
            ) {
                    ...
                    Handler().postDelayed({
                        if (isAdded && activity != null) {
                            presentation.showWouldYouLikeToAddTipScreen(
                                ...
                                prefProvider.getValueInt(PAYMENT_ID_FOR_CUSTOMER_DISPLAY, 0)
                            )
                        }
                    }, 5000)
    ...
    private fun getKitchenReceiptSettings() {
        viewModel.getKitchenReceiptSettings().observe(viewLifecycleOwner, Observer {
            ...
            if (it != null) {
                ...
                Log.e(
                    "getKitchenPrinters", "size of kitchen print list ${Gson().toJson(receiptModel)}"
                )
                if (!isFromCustomer) {
    ...
    private fun observeTipsList() {
        ...
        viewModelDashBoard.tipErrorObservable.observe(viewLifecycleOwner, object : Observer<String> {
                ...
                runOnUiThread(Runnable {
                    ...
                    context?.let {
                        ...
                        if (t.isNotEmpty()) {
                            ...
                            AlertUtils.showCustomAlertWithListenerWithOK(it, t, object : DialogInterface.OnClickListener {
                ...
                viewModelDashBoard.apply {
                    ...
                    if (paymentTypeForTip.equals("cash", true)) {
                        ...
                        if (!employeeGivenTip && changeAmount < 0.0 || changeAmount > 0.0) {
                            ...
                            val _title =
                                if (changeAmount < 0) "$" + /*Math.abs(changeAmount)*/ changeAmountFormatted.toString() + " to collect more" else "$" + /*Math.abs(changeAmount)*/ changeAmountFormatted.toString() + " Change"
...
//                            dashboardViewModel.setCollectMore(_title)
                    ...
                    } else if (paymentTypeForTip.lowercase(Locale.getDefault()).equals(
                            "card", true
                        ) && dashboardViewModel.customerGivenTipBefore.value == true
                    ) {

                        finalAmountToShow -= tipToShow

                        tipToShow += MethodUtils.calculateCashDiscount(
                            tipToShow, prefProvider, requireContext()
                        )
                    ...
                    binding.txtTipAmount?.text = "$${MethodUtils.roundOffAmountString(tipToShow)}"
                    binding.txtFinalAmount?.text = "$${
                        MethodUtils.roundOffAmountString(
                            finalAmountToShow
                        )
                    }"
...
//        ProgressUtils.showProgressDialog(requireActivity())
        ...
        if (isDineIn) {
            ...
            dineInList = requireArguments().getParcelableArrayList<DineInModel>(DINE_IN_ADAPTER_LIST) ?: arrayListOf()
        ...
        if (!isDineIn) {
            if (isSpilt) {
                ...
                viewModel.addSplitToDatabase(
                    title, (if (!isCustomCash) paidAmount + tipAmount else paidAmount) - splitChange, remainingAmount
                )
                binding.txtTitle.text = "$" + MethodUtils.roundOffAmountString(
                ...
                if (isSplitByAmount) {
                    if (paidAmount > WholetotalPrice) {
                        if (paymentType.equals("cash", true)) {
                            changeAmtGlobal = MethodUtils.roundOffAmountDouble(paidAmount - WholetotalPrice).toDouble()
                        }
                        if (paymentType.equals("cash", true)) {
                            binding.txtChangeAmount.visible()
                        }
                        ...
                        binding.txtPaymentAmount.text = "" + MainApplication.getInstance()!!.getText(R.string.symbole) + MethodUtils.roundOffAmountString(
                            paidAmount
                        ) + " payment successful"
                        ...
                        binding.txtChangeAmount.gone()
                        binding.txtChangeAmount.text = MethodUtils.roundOffAmount(0.0) + " Change"
                    ...
                    if (paidAmount > (amountWiseSplit + tipAmount)) {
                        if (paymentType.equals("cash", true)) {
                            changeAmtGlobal = MethodUtils.roundOffAmountDouble(paidAmount - (amountWiseSplit + tipAmount)).toDouble()
                        }
                        if (paymentType.equals("cash", true)) {
                            binding.txtChangeAmount.visible()
                        }
                    ...
                    if (isCustomCash && splitChange != 0.0) {
                        ...
                        binding.txtChangeAmount.text = MethodUtils.roundOffAmount(if (splitChange > 0) splitChange else 0.00) + " Change"
                        binding.txtPaymentAmount.text = "" + MainApplication.getInstance()!!.getText(R.string.symbole) + MethodUtils.roundOffAmountString(
                            paidAmount
                        ) + " payment successful"
                        ...
                        var changeValue = 0.0
                        if (prefProvider.getValue(OPTION_TYPE, "") == "SurCharge") {
                            changeValue = (paidAmount - prefProvider.getValue(CASH_DISCOUNT_SURCHARGE, "").toDouble()) - remainingAmount
                        } else {
                            changeValue = paidAmount - remainingAmount
                        }
                        if (Math.round(changeValue) > 0.0) {
                            if (paymentType.equals("cash", true)) {
                                changeAmtGlobal = MethodUtils.roundOffAmountDouble(changeValue).toDouble()
                            }
                            if (paymentType.equals("cash", true)) {
                                binding.txtChangeAmount.visible()
                            }
                            binding.txtChangeAmount.text = MethodUtils.roundOffAmount(if (changeValue > 0) changeValue else 0.00) + " Change"
                ...
                if (isCustomCash) {
                    binding.txtTitle.text = "$" + MethodUtils.roundOffAmountString(paidAmount)
                    binding.txtPaymentAmount.text =
                        "" + MainApplication.getInstance()!!.getText(R.string.symbole) + MethodUtils.roundOffAmountString(paidAmount) + " payment successful"

                } else {
                    binding.txtTitle.text = "$" + MethodUtils.roundOffAmountString(paidAmount + tipAmount)
                    binding.txtPaymentAmount.text = "" + MainApplication.getInstance()!!
                ...
                if (isCustomCash) {
                    ...
                    binding.txtChangeAmount.text = MethodUtils.roundOffAmount(if (ca > 0) ca else 0.00) + " Change"
//                    --------------------------------------------------------------------
...
//                    --------------------------------------------------------------------
            ...
            if (isSpilt) {
                ...
                viewModel.addSplitToDatabase(
                    title, (paidAmount + tipAmount) - splitChange, remainingAmount
                )
                binding.txtTitle.text = "$" + MethodUtils.roundOffAmountString(
                    paidAmount + tipAmount
                )
                ...
                if (remainingAmount < paidAmount) {
                    ...
                    if (isCustomCash && splitChange != 0.0) {
                        ...
                        var temp_Change = MethodUtils.roundOffAmountDouble((paidAmount - noCashAdjGlobal) - remainingAmount)
                        if (!(temp_Change.equals(0.0) || temp_Change.equals(0) || temp_Change <= 0.0)) {
                            changeAmtGlobal = MethodUtils.roundOffAmountDouble((paidAmount - noCashAdjGlobal) - remainingAmount)
                            if (paymentType.equals("cash", true)) {
                                binding.txtChangeAmount.visible()
                            }
...
//        }
    ...
    private fun cloudQueuePrinting(
        input: String, shop_id: Int, data: PrinterResponse.Data.KitchenReceiptPrinters
    ) {
        val date = Date()
        val random = Random()
...
//        setCharacterSize(1,1)
        ...
        appendText(
            "${
                getReceiptFormatDateFromUTCServer(
                    requireContext(), receiptModel?.order?.createdAt.toString()
                )
            }"
        )
        lineFeed(1)
        ...
        if (orderItems.isNotEmpty() && orderItems != null) if (prefProvider.getValueboolean(OPEN_ORDER_UPDATE_FOR_PRINT, false) == true) {
            ...
            for (i in 0 until printOrderItems.size) {
                ...
                kitchenReceiptPrinters.printerCategories.toCollection(arrayListOf()).forEach {
                    Log.e(
                        "PrinterReceipt", "checkPrinterItemN:   ${printOrderItems.get(i).itemName}"
                    )
        ...
        if (receiptModel?.order?.customer != null) {
            ...
            if (receiptModel?.order?.customer?.phones?.isNotEmpty() == true) {
                var phoneNo = receiptModel?.order?.customer?.phones?.size?.minus(1)?.let { receiptModel?.order?.customer?.phones?.get(it)?.phoneNumber }
        ...
        Log.e(
            "checkKey", "pushContent: checkSN:${input} ${
                pushContent(
                    trade_no = String.format("%s_%010d", "${input}", System.currentTimeMillis()), "${input}", 1, 1, "您有新的订单", 0
                )
            }"
        )
    ...
    private fun observeTipClicked() {
        dashboardViewModel.tipButtonOnCustomerDisplayClicked.observe(viewLifecycleOwner, object : Observer<Boolean> {
    ...
    private fun setUpdateEnabledInReceiptModel() {
        if (prefProvider.getValue(Constants.OPEN_ORDER_ITEMS_OLD, "").isNotEmpty()) {
            if (prefProvider.getValue(Constants.GIFT_CARD_TYPE, "")
                    .isEmpty()
            ) { //this block was executed in case of gift card also, hence we are applying the restriction
                var oldDataModel: List<OnlineOrderResponseModel.Data.OrderItem> = Gson().fromJson(
                    prefProvider.getValue(Constants.OPEN_ORDER_ITEMS_OLD, ""),
                    object : TypeToken<List<OnlineOrderResponseModel.Data.OrderItem>?>() {}.type
                )

                var (newData, isUpdated) = getPrintingData(
                    oldDataModel, receiptModel?.order?.orderItems
                )
    ...
    fun getPrintingData(
        oldItems: List<OnlineOrderResponseModel.Data.OrderItem>, newItems: List<CreateOrderResponse.Data.Order.OrderItem>?
...
//                                }
    ...
    private fun setLabelData() {
        var paymentTypeTitle: java.lang.StringBuilder? = java.lang.StringBuilder()/* if (paymentType == "Card") {
...
*/
                ...
                binding.txtAddCustomer.visibility = View.GONE
//                binding.viewSplitLine.visibility = View.GONE
                binding.tvMessage.text = getString(R.string.please_enter_customer_contact_number)
  ...
  */
                ...
                binding.txtAddCustomer.visibility = View.GONE
                binding.llOptions.visibility = View.GONE
                binding.tvMessage.text = getString(R.string.please_enter_customer_email_address)
                MethodUtils.hideKeyboard(requireActivity())
            }
            ...
            R.id.txtSend -> {
                Log.d(TAG2, "Start printer OrderCompleteFragment.kt, R.id.txtSend")
                EventBus.getDefault().post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragment.kt, R.id.txtSend"))

                MethodUtils.hideKeyboard(requireActivity())
                viewModel.submit(
                    type, binding.edtEmail.text.toString().trim(), binding.edtPhoneNo.text.toString().trim(), orderID, IS_GIFT_CARD_TYPE
                )
    ...
    private fun initDineInPrinter(
        ...
        if (customerReceiptPrinters.name.startsWith("CloudPrint", true)) {
            ...
            if (isGuest) {
                ...
                LogUtil.logE(
                    TAG, "checkOutDineInModel:  ${Gson().toJson(checkOutDineInModel)}"
                )


                if (checkOutDineInModel != null) {
                    generateGuestPrint(
                        customerReceiptPrinters, type, paymentType, listGuestItem, guestName, listWTitems, checkOutDineInModel
                    )
                }
            ...
            if (isGuest) {
                ...
                generateDineInPrintForLandi(customerReceiptPrinters, type, "")
                //printDineInTableCommon(CommonPrinterTypes.LandiInnerPrinter,customerReceiptPrinters)
            }
        } else {
...
//                printer.setBatteryStatusChangeEventCallback(this)
                ...
                try {
                    ...
                    printer?.openPrinter(
                        ...
                        if (customerReceiptPrinters.printer_type == BLUETOOTH) {
                            ...
                            Print.DEVTYPE_TCP
                        }, customerReceiptPrinters.ipAddress, enabled, interval
                    )
                    //printer?.setStatusChangeEventCallback(this)
    ...
    private fun generateGuestPrint(
            ...
            it.taxes?.forEach { tax ->
                if (tax.isActive) {
                    ...
                    guestTaxes += if (tax.taxType == "Percentage") {
                        ...
                        val price = (it.price * it.itemQuantity) - it.discountPrice
                        ...
                        String.format("%.2f", tax.rate * it.itemQuantity).toDouble()
                    }
                }
        ...
        if (customerReceiptPrinters.name.startsWith(SUNMI_PRINTER, true)) {
            ...
            try {
                builder = Builder(
                    if (customerReceiptPrinters.name.substring(0, 6).toString().lowercase() == "TM-m30".lowercase()) {
                        "TM-m30"
                    } else {
                ...
                if (customerSettingModel.showVenueLogo && prefProvider.getValue(VENUE_LOGO, "").isNotEmpty()) {
                    builder.addFeedLine(1)
                    builder.addTextAlign(Builder.ALIGN_CENTER)
                    ...
                    val decodedString: ByteArray = Base64.decode(
                        prefProvider.getValue(VENUE_LOGO, ""), Base64.DEFAULT
                    )
                    ...
                    builder.addImage(
                        newBitmap, 0, 0, newBitmap.width, newBitmap.height, Builder.COLOR_1, Builder.MODE_MONO, Builder.HALFTONE_DITHER, 1.0
                    )
                }
                ...
                addBuilderText(
                    builder, prefProvider.getValue(BUSINESS_NAME, "").toString()
                )
                if (customerSettingModel.showVenueAddress) {
                    ...
                    addBuilderText(
                        builder, prefProvider.getValue(BUSINESS_ADDRESS, "").toString()
                    )
                }
                if (customerSettingModel.showVenuePhone) {
                    ...
                    addBuilderText(
                        builder, MethodUtils.formatPhoneNumber(
                ...
                if (customerSettingModel.showWebsiteAddress) {
                    ...
                    addBuilderText(
                        builder, prefProvider.getValue(BUSINESS_WEBSITE, "").toString()
                    )
                }
                    ...
                    if (customerSettingModel.showOrderTime) {
                        ...
                        builder.addText(
                            "Order Time:" + getReceiptFormatDateFromUTCServer(
                                requireContext(), getDineInOrderDetails?.createdAt.toString()
                            )
                        )
                ...
                if (listGuestItem.isNotEmpty()) {
                    ...
                    listGuestItem.forEach {
                        addOrderItemForDineIn(
                            builder, it, customerSettingModel.fonts, customerSettingModel.showModifiers
                        )
...
//                                "-$" + MethodUtils.roundOffAmountString(0.0)
                ...
                builder.addText(
                    padLine(
                        "Sub Total", "$" + MethodUtils.roundOffAmountString(
                ...
                if (guestTaxes != null) {
                    ...
                    builder.addText(
                        padLine(
                            "Tax", "$" + MethodUtils.roundOffAmountString(checkOutDineInModel.totalTax), if (customerSettingModel.fonts == LARGE) {
                                24
                            } else {
                                48
                            }
                ...
                if ((getDineInOrderDetails?.serviceChargeEnabled == true) && prefProvider.getValueboolean(
                        SERVICECHARGE_DINEIN_ORDER, false
                    )
                ) {
                    builder.addTextLineSpace(30)
                    builder.addFeedUnit(30)
                ...
                if (tipAmount != 0.0) {
                    ...
                    builder.addText(
                        padLine(
                            "Tip", "$" + MethodUtils.roundOffAmountString(tipAmount), if (customerSettingModel.fonts == LARGE) {
                                24
                            } else {
                                48
                            }
                ...
                if (noCashAdjGlobal != 0.0 && customerSettingModel.showCashDisSurCharg) {
                    ...
                    if (prefProvider.getValue(OPTION_TYPE, "").lowercase() == "CashDiscount".lowercase() && payTypeGlb == "Cash") {

                        builder.addText(
                            padLine(
                                "Cash Discount", if (noCashAdjGlobal == 0.0) {
                    ...
                    } else if (prefProvider.getValue(OPTION_TYPE, "").lowercase() == "SurCharge".lowercase() && payTypeGlb == "Card") {
                        builder.addText(
                            padLine(
                                Constants.SURCHARGE_TEXT, if (noCashAdjGlobal == 0.0) {
                                    "$" + MethodUtils.roundOffAmountString(noCashAdjGlobal)
                                } else {
                                    "$" + MethodUtils.roundOffAmountString(noCashAdjGlobal)
                                }, if (customerSettingModel.fonts == LARGE) {
                ...
                builder.addText(
                    padLine(
                        "Total Price", "$" + MethodUtils.roundOffAmountString(totalAmt), if (customerSettingModel.fonts == LARGE) {
                            24
                        } else {
                            48
                        }
                ...
                builder.addText(
                    padLine(
                        "Paid Amount", "$" + MethodUtils.roundOffAmountString(
                            paidAmount + tipAmount
                        ), if (customerSettingModel.fonts == LARGE) {
                            24
                        } else {
                ...
                if (payTypeGlb == "Cash") {
                    builder.addText(
                        padLine(
                            "Change Amount", "$" + MethodUtils.roundOffAmountString(changeAmtGlobal), if (customerSettingModel.fonts == LARGE) {
                                24
                            } else {
                                48
                            }
                ...
                if (customerSettingModel.showTipSuggestion) {
                    ...
                    builder.addText(
                        padLine(
                            "Additional Tips", "", if (customerSettingModel.fonts == LARGE) {
                                24
                            } else {
                                48
                            }
                    ...
                    if (tipsList.isNotEmpty()) {
                        addTipsList(
                            ...
                            MethodUtils.roundOffAmountDouble(guestSubTotal + guestServiceCharge + guestTaxes), customerSettingModel.fonts
                        )
                ...
                if (receiptModel?.order?.payments?.isNotEmpty() == true) {
                    ...
                    builder.addText(
                        padLine(
                            "Transaction ID",
                            "" + receiptModel?.order?.payments?.size?.minus(1)?.let { receiptModel?.order?.payments?.get(it)?.id },
                            if (customerSettingModel.fonts == LARGE) {
                                24
                            } else {
                                48
                ...
                if (receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.paymentType?.lowercase() == "Card".lowercase()) {
                    ...
                    var strCardType = receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.cardType
                    ...
                    if (paymentViewModel.extData != null && !paymentViewModel.extData.isNullOrEmpty()) {
                        ...
                        strCardType = paymentViewModel.extData.substring(
                            applabStartIndex + "<APPLAB>".length, applabEndIndex
                        )
                    }

                    builder.addText(
                        padLine(
                            "", strCardType, if (customerSettingModel.fonts == LARGE) {
                                24
                            } else {
                                48
                            }
                ...
                builder.addText(
                    padLine(
                        "Customer Signature", addHorizontalHalfCustomerReceiptLine(customerSettingModel.fonts), if (customerSettingModel.fonts == LARGE) {
                            24
                        } else {
                            48
                        }
                ...
                if (customerSettingModel.showQrCode) {
                    builder.addFeedLine(1)
                    builder.addTextAlign(Builder.ALIGN_CENTER)
                    val bitmap = generateQRCode(getDineInOrderDetails?.digitalReceiptUrl.toString())
                ...
                try {
                    ...
                    PrinterClass.getPrinter()?.sendData(
                        builder, BLUETOOTH_TIMEOUT, status, battery
                    )
    ...
    private fun sunmiPrinterInit(
        ...
        connect(
            paymentType, listWTitems, listGuestItem, guestName, finaldisLocal, checkOutDineInModel, orderDiscount, guestTaxes, guestSubTotal, guestServiceCharge
        )
    ...
    private fun connect(
        ...
        if (!SunmiPrinterApi.getInstance().isConnected) {
            SunmiPrinterApi.getInstance().connectPrinter(requireContext(), object : ConnectCallback {
    ...
    private fun printSunmiDinein(
        ...
        try {
            ...
            PrintSunmiUtils.printBusinessDetails(
                prefProvider.getValue(BUSINESS_NAME, ""), if (customerSettingModel.showVenueAddress) prefProvider.getValue(
                    BUSINESS_ADDRESS, ""
                ) else "", if (customerSettingModel.showVenuePhone) prefProvider.getValue(
                    BUSINESS_PHONE_NO, ""
                ) else ""
            )
            ...
            if (customerSettingModel.fonts == LARGE) {
                ...
                if (customerSettingModel.showPrintTime) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                        PrintSunmiUtils.orderTime(
                            "Print Time:" + getCurrentTimeFromTimeZone(
                                requireContext(), MethodUtils.formatted()
                            )
                        )
                    }
            ...
            if (listGuestItem.isNotEmpty()) {
                ...
                listGuestItem.forEach {
                    addOrderItemForDineIn(
                        it, customerSettingModel.fonts, customerSettingModel.showModifiers
                    )
...
//                        "-$" + MethodUtils.roundOffAmountString(0.0)
            ...
            if (guestTaxes != null) {
                ...
                val str3 = padLine(
                    "Tax", "$" + MethodUtils.roundOffAmountString(checkOutDineInModel.totalTax), if (customerSettingModel.fonts == LARGE) 23 else 48
                ).toString()
            ...
            if (tipAmount != 0.0) {
                ...
                val str8 = padLine(
                    "Tip", "$" + MethodUtils.roundOffAmountString(tipAmount), if (customerSettingModel.fonts == LARGE) 23 else 48
                ).toString()
            ...
            val str5 = padLine(
                "Total Price", "$" + MethodUtils.roundOffAmountString(totalAmt), if (customerSettingModel.fonts == LARGE) 23 else 48
            ).toString()
            ...
            if (payTypeGlb == "Cash") {
                val str7 = padLine(
                    "Change Amount", "$" + MethodUtils.roundOffAmountString(changeAmtGlobal), if (customerSettingModel.fonts == LARGE) 23 else 48
                ).toString()
            ...
            if (customerSettingModel.showTipSuggestion) {
                ...
                if (tipsList.isNotEmpty()) {
                    addTipsList(
                        tipsList, MethodUtils.roundOffAmountDouble(guestSubTotal + guestServiceCharge + guestTaxes), customerSettingModel.fonts
                    )
            ...
            if (receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.paymentType?.lowercase() == "Card".lowercase()) {
                ...
                PrintSunmiUtils.normalText(
                    padLine(
                        "", strCardType, if (customerSettingModel.fonts == LARGE) 23 else 48
                    ).toString()
                )
                PrintSunmiUtils.normalText(
                    padLine(
            ...
            SunmiPrinterApi.getInstance().lineWrap(2)
            val str8 = padLine(
                "Customer Signature", "     _________________________", 48
            ).toString()
    ...
    private fun printLandiDineinInner(
        paymentType: String,
        listWTitems: ArrayList<TbCartItem>,
        listGuestItem: ArrayList<TbCartItem>,
        guestName: String,
        ...
        val order = getDineInOrderDetails
                                    ...
                                    try {
                                        ...
                                        if (customerSettingModel.showOrderIdTop) {
                                            val orderIdToPrint = if (prefProvider.getValueboolean(
                                                    ORDER_NUMBER_STARTING_FROM_ONE, false
                                                )
                                            ) {
                                            ...
                                            printCenter(
                                                orderIdToPrint, FONT_SIZE_5X, isBold = true
                                            )

                                            printCenter(
                                                "Paid", FONT_SIZE_5X, isBold = true, printOnNewLine = true
                                            )
                                        }
                                            ...
                                            try {
                                                ...
                                                if (Build.DISPLAY.contains("RL")) {
                                                    landiPrinter.addImage(
                                                        venueUrlByteArray, Align.RIGHT, 0
                                                    )
                                                } else {
                                                    landiPrinter.addImage(
                                                        venueUrlByteArray, Align.CENTER, 0
                                                    )
                                                }
                                                ...
                                                Log.d(
                                                    "DMJ", "Error getting image bytes to print"
                                                )
                                            }
                                        ...
                                        printCenter(
                                            prefProvider.getValue(
                                                BUSINESS_NAME, ""
                                            ), fontSize = FONT_SIZE_DOUBLE_HEIGHT, isBold = true, printOnNewLine = true
                                        )
                                        ...
                                        printCenter(
                                            "Dine In", isBold = true, fontSize = FONT_SIZE_5X
                                        )
                                        lineBreak()
                                        ...
                                        printLeft(
                                            "Order Time : ${
                                                getReceiptFormatDateFromUTCServer(
                                                    requireContext(), order?.createdAt.toString()
                                                )
                                            }"
                                        )
                                        ...
                                        var guestCount: Int = (order?.guestAttributes?.size?.minus(1)) ?: 1
                                        if (guestCount < 1) {
                                            guestCount = 1
                                        }
                                        ...
                                        listGuestItem.forEach {
                                            addOrderItemForDineInInnerLandi(
                                                it, customerSettingModel.fonts, customerSettingModel.showModifiers, LPrint
                                            )
                                        ...
                                        if (order?.totalDiscount != null) {
                                            ...
                                            val discountToPrint = padLine(
                                                "Total Discount", (if (guestDiscount != 0.0) "-$" + MethodUtils.roundOffAmountString(
                                                    guestDiscount
                                        ...
                                        try {
                                            fetchSubTotalFromPreference = prefProvider.getValue(
                                                DINE_IN_SUB_TOTAL_AMOUNT_BEFORE_PAYMENT_GUEST, fetchSubTotalFromPreference.toString()
                                            ).toDouble()
                                        } catch (e: Exception) {
                                        ...
                                        if (guestTaxes != null) {
                                            ...
                                            val taxToPrint = padLine(
                                                "Tax", "$" + MethodUtils.roundOffAmountString(
                                        ...
                                        if (guestServiceCharge != null && (order?.serviceChargeEnabled == true) && prefProvider.getValueboolean(
                                            ...
                                            val serviceChargeToPrint = padLine(
                                                "Service Charge", "$" + MethodUtils.roundOffAmountString(
                                        ...
                                        if (tipAmount != 0.0) {
                                            ...
                                            val str8 = padLine(
                                                "Tip", "$" + MethodUtils.roundOffAmountString(tipAmount), 48
                                            ).toString()
                                            lineBreak()
                                            printLeft(str8)
                                        }
                                        ...
                                        val str5 = padLine(
                                            "Total Price", "$" + MethodUtils.roundOffAmountString(
                                                (guestPayment!!.amount + tipAmount)
                                            ), 48
                                        ).toString()
                                        ...
                                        if (guestPayment.paymentType == "Cash") {
                                            ...
                                            if (changeAmount >= 0.0) {
                                                val str7 = padLine(
                                                    "Change Amount", MethodUtils.roundOffAmount(changeAmount), 48
                                                ).toString()
                                        ...
                                        if (customerSettingModel.showTipSuggestion) {
                                            ...
                                            if (tipsList.isNotEmpty()) {
                                                val tipsToPrint = addTipsListInnerLandi(
                                                    tipsList, newPaidAmount, customerSettingModel.fonts
                                                )
                                        ...
                                        if (order.payments.isNotEmpty()) {
                                            ...
                                            val str10 = padLine(
                                                "Transaction ID", "" + order.payments.size.minus(1).let { order.payments[it].id }, 48
                                            ).toString()
                                            printLeft(str10)
                                            lineBreak()

                                            val str11 = padLine(
                                                "Transaction Type", order.payments[order.payments.size - 1].paymentType, 48
                                            ).toString()
                                            printLeft(str11)
                                            lineBreak()
                                        }
                                        ...
                                        if (customerSettingModel.showQrCode) {
                                            printQRCode(
                                                outputStream, order.digitalReceiptUrl.toString(), CENTER_ALIGN
                                            )
                                        }
    ...
    private fun printSunmiDineinInner(
        ...
        try {
            ...
            PrintSunmiUtils.printBusinessDetailsInner(
                prefProvider.getValue(BUSINESS_NAME, ""), if (customerSettingModel.showVenueAddress) {
            ...
            val guestCount = getDineInOrderDetails?.guestAttributes?.count {
                it.guestItemAttributes.isNotEmpty() && !it.name.lowercase().contains("whole table")
            } ?: 1
            ...
            if (listGuestItem.isNotEmpty()) {
                ...
                listGuestItem.forEach {
                    addOrderItemForDineInInner(
                        it, customerSettingModel.fonts, customerSettingModel.showModifiers, prefProvider.isOldSunmiFrameworkVersion()
                    )
...
//                        "-$" + MethodUtils.roundOffAmountString(0.0)
            ...
            PrintSunmiUtils.printBoldText(prefProvider.isOldSunmiFrameworkVersion(), str6)
            ...
            if (receiptModel?.order?.payments?.isNotEmpty() == true) {
                ...
                PrintSunmiUtils.printNormalText(prefProvider.isOldSunmiFrameworkVersion(), str11)
            }
            ...
            if (receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.paymentType?.lowercase() == "Card".lowercase()) {
                ...
                PrintSunmiUtils.printNormalText(
                    prefProvider.isOldSunmiFrameworkVersion(), padLine(
                        "", strCardType, if (customerSettingModel.fonts == LARGE) 23 else 48
                    ).toString()
                )
                PrintSunmiUtils.printNormalText(
            ...
            if (customerSettingModel.fonts == LARGE) {
                PrintSunmiUtils.printBoldText(
                    prefProvider.isOldSunmiFrameworkVersion(), "Customer Signature ____"
                )
            } else {
                PrintSunmiUtils.printBoldText(
                    prefProvider.isOldSunmiFrameworkVersion(), "Customer Signature           __________________"
                )
            }
    ...
    fun checkMaxGuestCountId(serviceChargeList: ArrayList<TbServiceCharge>): Int {
        ...
        serviceChargeList.forEach { serviceCharge ->
            if (serviceCharge.order_type == SERVICECHARGE_DINEIN_ORDER) {
    ...
    private fun generateDineInPrint(
        customerReceiptPrinters: PrinterResponse.Data.CustomerReceiptPrinters, type: String, s: String
    ) {

        if (customerReceiptPrinters.name.startsWith("CloudPrint", true)) {


            SunmiPrinterApi.getInstance().setPrinter(SunmiPrinter.SunmiBlueToothPrinter, customerReceiptPrinters.ipAddress)
            ...
            try {
                ...
                addBuilderText(
                    builder, prefProvider.getValue(BUSINESS_ADDRESS, "7450 DW 51 FH,AT,Suite 503").toString()
                )
                builder.addFeedLine(1)
                    ...
                    if (dineInList[i].isHeader == 0) {
                        ...
                        if (i != (dineInList.size - 1) && dineInList[i + 1].isHeader == 1) {
                            ...
                            if (dineInList[i].customer == null) {
                                ...
                                builder.addText(
                                    dineInList[i].customer?.first_name + " " + if (dineInList[i].customer?.last_name != null) {
                ...
                if (getDineInOrderDetails?.totalDiscount != null) {
                    ...
                    builder.addText(padLine("Total Discount",
...
//                                "-$" + MethodUtils.roundOffAmountString(0.0)
                ...
                if (noCashAdjGlobal != 0.0 && payTypeGlb == "Cash" && prefProvider.getValue(
                    ...
                    builder.addText(
                        padLine(
                            Constants.SURCHARGE_TEXT, "" + MethodUtils.roundOffAmount(
                                receiptModel?.order?.payments?.get(
                                    receiptModel?.order?.payments?.size!! - 1
                ...
                var totalAmt = MethodUtils.roundOffAmountDouble(
                ...
                if (isSpilt) {
                    ...
                    builder.addText(
                        padLine(
                            "Remaining Amount", "$" + MethodUtils.roundOffAmountString(remainingAmount), if (customerSettingModel.fonts == LARGE) {
                                24
                            } else {
                                48
                            }
                ...
                if (getDineInOrderDetails?.totalTips == 0.0) {
                    ...
                    builder.addText(
                        padLine(
                            "Tip", if (customerSettingModel.showTipLineForCash) {
                                "_____________"
                            } else {
                                ""
                            }, if (customerSettingModel.fonts == LARGE) {
                    ...
                    builder.addText(
                        padLine(
                            "Total", if (customerSettingModel.showTipLineForCash) {
                                "_____________"
                            } else {
                                ""
                            }, if (customerSettingModel.fonts == LARGE) {
                ...
                if (customerSettingModel.showTipSuggestion) {
                    ...
                    if (tipsList.isNotEmpty()) {
                        addTipsList(
                            builder, tipsList, receiptModel?.order?.totalAmount?.toDouble() ?: 0.0, customerSettingModel.fonts
                        )
...
//                                    }?.let { PrintSunmiUtils.printTextCenter(it) }
            ...
            if (getDineInOrderDetails?.totalDiscount != null) {
                ...
                val str1 = padLine("Total Discount",
...
//                        "-$" + MethodUtils.roundOffAmountString(0.0)
            ...
            if (isSpilt) {
                ...
                val str7 = padLine(
                    "Remaining Amount", "$" + MethodUtils.roundOffAmountString(remainingAmount), if (customerSettingModel.fonts == LARGE) 23 else 48
                ).toString()
                PrintSunmiUtils.changeAmount(str7)
            ...
            if (customerSettingModel.showTipSuggestion) {
                ...
                if (tipsList.isNotEmpty()) {
                    addTipsList(
                        tipsList, receiptModel?.order?.totalAmount?.toDouble() ?: 0.0, customerSettingModel.fonts
                    )
            ...
            if (receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.paymentType?.lowercase() == "Card".lowercase()) {
                ...
                val str12 = receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.cardName.toString()


                var str13 = receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.cardType.toString()
                ...
                if (paymentViewModel.extData != null && !paymentViewModel.extData.isNullOrEmpty()) {
                    ...
                    var applabStartIndex = paymentViewModel.extData.indexOf("<APPLAB>")
                    var applabEndIndex = paymentViewModel.extData.indexOf("</APPLAB>")
                    str13 = paymentViewModel.extData.substring(
                        applabStartIndex + "<APPLAB>".length, applabEndIndex
                    )
                }


                val str14 = receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.cardNumber.toString()
...
//            SunmiPrinterApi.getInstance().lineWrap(1)
    ...
    private fun generateDineInPrintForLandi(
        customerReceiptPrinters: PrinterResponse.Data.CustomerReceiptPrinters, type: String, s: String
    ) {
                            ...
                            outputStream.apply {
                                ...
                                if (customerSettingModel.showOrderIdTop) {
                                    if (prefProvider.getValueboolean(
                                        ...
                                        outputStream.write("OrderID: ${order?.custom_order_id}".toByteArray())
                                    } else {
                                        outputStream.write("OrderID: ${order?.id}".toByteArray())
                                    }
                                    outputStream.write(LPrint.LINE_FEED)
                                }

                                outputStream.write("Paid".toByteArray())
                                outputStream.write(LPrint.LINE_FEED)
...
//                                        )
                                ...
                                outputStream.apply {
                                    ...
                                    write(LPrint.FONT_SIZE_5X)
                                    write("Dine In".toByteArray())

                                    write(LPrint.BOLD_OFF)
                                ...
                                outputStream.apply {
                                    ...
                                    write("Employee : ${prefProvider.employeeName()}".toByteArray())
                                    write(LPrint.LINE_FEED)
                                        ...
                                        if (dineInList[i].isHeader == 0) {
                                            ...
                                            if (i != (dineInList.size - 1) && dineInList[i + 1].isHeader == 1) {
                                                ...
                                                if (dineInList[i].customer == null) {
                                                    ...
                                                    val tableName = dineInList[i].customer?.first_name + " " + if (dineInList[i].customer?.last_name != null) {
                                                        dineInList[i].customer?.last_name
                                                    } else {
                                                        ""
                                                    }
                                            ...
                                            dineInList.get(i).item?.let { item ->
                                                ...
                                                val itemDataToPrint = padLineCustomerItem(
                                                    obj.itemQuantity.toString() + "  " + getItemNameToShow(
                                                        obj.name
                                                    ), getItemPriceToShow(obj.price * obj.itemQuantity), 48
                                                ).toString().toByteArray()


                                                write(LPrint.LEFT_ALIGN)
                                                write(itemDataToPrint)
                                                write(LPrint.LINE_FEED)


                                                if (obj.modifiers.isNotEmpty() && customerSettingModel.showModifiers) {
                                                    for (j in 0 until obj.modifiers.size) {
                                                        val modifierObj = obj.modifiers.get(j)


                                                        val modifiersToPrint = padLineCustomerItem(
...
//                                                                    },
                                                            "  " + modifierObj.modifier_quantity + "x " + getItemNameToShow(
                                                                ...
                                                                modifierObj.price, obj.itemQuantity * modifierObj.modifier_quantity
                                                            ), 48
                                                        ).toString().toByteArray()


                                                        write(modifiersToPrint)
                                                        write(LPrint.LINE_FEED)
                                ...
                                if (order?.totalDiscount != null) {
                                    ...
                                    val discountToPrint = padLine("Total Discount",
...
//                            "-$" + MethodUtils.roundOffAmountString(0.00)
                                        ...
                                        }, 48).toString().toByteArray()

                                    outputStream.write(discountToPrint)
                                    outputStream.write(LPrint.LINE_FEED)
                                }
                                ...
                                try {
                                    fetchSubTotalFromPreference = prefProvider.getValue(
                                        DINE_IN_SUB_TOTAL_AMOUNT_BEFORE_PAYMENT, fetchSubTotalFromPreference.toString()
                                    ).toDouble()
                                } catch (e: Exception) {
                                ...
                                var subTotalAmount = if (splitAmount != 0.0) "($" + MethodUtils.roundOffAmountString(splitAmount) + ") "
                                else ""
                                ...
                                outputStream.write(subTotalToPrint)
                                outputStream.write(LPrint.LINE_FEED)
                                ...
                                if (serviceCharge != null && (order?.serviceChargeEnabled == true) && prefProvider.getValueboolean(
                                        SERVICECHARGE_DINEIN_ORDER, false
                                    )
                                ) {
                                    ...
                                    outputStream.write(serviceChargeToPrint)
                                    outputStream.write(LPrint.LINE_FEED)
                                ...
                                if (tipAmount != 0.0) {
                                    ...
                                    val tipsToPrint = padLine(
                                        "Tip", "$" + MethodUtils.roundOffAmountString(
                                            tipAmount
                                        ), 48
                                    ).toString().toByteArray()
                                ...
                                totalAmt += serviceCharge
                                totalAmt += /*order?.totalTaxAmount*/paidTax
...
//                                    MethodUtils.roundOffAmountString(totalAmt)
                                    val str5 = padLine(
                                        "Total Price", "$" + order?.payments?.last()?.let {
                                            MethodUtils.roundOffAmountString(
                                                it.amount + it.tips
                                            )
                                        }, 48
                                    ...
                                    Log.e(
                                        "PRINTING ERROR WHILE DINE IN _ LANDI INNER PRINTER", "Total price printing" + e.message.toString()
                                    )
                                }
                                ...
                                try {
                                    if (_order?.payments?.last()?.paymentType == "Cash") {
                                        ...
                                        val changeAmount = (paidAmount) - (order?.payments?.last()?.amount!!.plus(
                                            tipAmount
                                        ))
...
//                                        ).toString().toByteArray()
                                ...
                                if (isSpilt) {
                                    ...
                                    val str7 = padLine(
                                        "Remaining Amount", "$" + MethodUtils.roundOffAmountString(remainingAmount), 48
                                    ).toString().toByteArray()
                                    write(LPrint.LINE_FEED)
                                    write(str7)
                                    write(LPrint.LINE_FEED)
                                ...
                                if (tipAmount == 0.0) {
                                    ...
                                    if (customerSettingModel.showTipLineForCash) {
                                        ...
                                        write(LPrint.BOLD_ON)
                                        write("Tip                               _____________".toByteArray())
                                        write(LPrint.LINE_FEED)
                                        write("Total                             _____________".toByteArray())
                                        write(LPrint.BOLD_ON)
                                ...
                                if (getDineInOrderDetails?.payments?.isNotEmpty() == true) {
                                    ...
                                    if (payments?.first()?.paymentType == "Guest" || payments?.size ?: 0 > 1) {
                                        ...
                                        printPayment(
                                            false, outputStream, LANDI_INNER_PRINTER, list = getDineInOrderDetails!!.payments
                                        )
                                    }
                                }
                                ...
                                if (customerSettingModel.showTipSuggestion) {
                                    ...
                                    if (tipsList.isNotEmpty()) {
                                        val tipsToPrint = addTipsListInnerLandi(
                                            tipsList, amount, customerSettingModel.fonts
                                        )

                                        write(tipsToPrint.toByteArray())
                                        write(LPrint.LINE_FEED)
                                    }
                                ...
                                val getOrderDetailsResponse = order
                                ...
                                if (getOrderDetailsResponse?.payments?.isNotEmpty() == true && type.lowercase() != "unpaid") {
                                    ...
                                    LPrint.printLeft(
                                        padLine(
                                            "Transaction ID", payment.id.toString(), 48
                                        ).toString()
                                    )

                                    LPrint.printLeft(
                                        padLine(
                                            "Transaction Type", payment.paymentType, 48
                                        ).toString()
                                    )
                                    ...
                                    try {
                                        ...
                                        if (payment.paymentType.lowercase() == "Card".lowercase()) {/*val str12 = padLine(
                                                    "",
                                            ...
                                            var strCardType = payment.cardType.toString()
                                            ...
                                            val str14 = padLine(
                                                "", payment.cardNumber.toString(), 48
                                            ).toString()
                                        ...
                                        Log.e(
                                            "PRINTING ERROR WHILE DINE IN _ LANDI INNER PRINTER", "PAYMENTS printing issue" + e.message.toString()
                                        )
                                    }
                                }
...
//                                    write(LPrint.LINE_FEED)
                                ...
                                if (order?.note != null && order.note != "" && customerSettingModel.showOrderNote) {
                                    ...
                                    printCenter(
                                        "Order Note\n${order.note}", printOnNewLine = true
                                    )
                                    lineBreak()
//                                        printCenter()
                                    lineBreak()
                                ...
                                if (customerSettingModel.showQrCode) {
                                    ...
                                    LPrint.printQRCode(
                                        outputStream, order?.digitalReceiptUrl.toString(), LPrint.CENTER_ALIGN
                                    )
                                ...
                                /**
                                 * CUT PAPER AND DISCONNECT PRINTER
                                 */
                                write(LPrint.CUT_PAPER)
                            ...
                            printingCustomer = false
                            Log.e(
                                "PRINTING ERROR WHILE DINE IN _ LANDI INNER PRINTER", e.printStackTrace().toString() + " " + e.message.toString()
                            )
                        }
                    }
                }
    ...
    private fun printDineInTableCommon(
        printerType: CommonPrinterTypes, customerReceiptPrinters: PrinterResponse.Data.CustomerReceiptPrinters? = null
    ) = try {
        ...
        if (customerSettingModel.showOrderIdTop) {
            if (prefProvider.getValueboolean(ORDER_NUMBER_STARTING_FROM_ONE, false)) {
                printerTasks.add(
                    Pair(
                        ORDER_ID, "OrderID:" + getDineInOrderDetails?.custom_order_id
                    )
                )
            } else {
        ...
        printerTasks.add(Pair(BUSINESS_PHONE_NO, prefProvider.getValue(BUSINESS_PHONE_NO, "")))
        printerTasks.add(Pair(BUSINESS_WEBSITE, prefProvider.getValue(BUSINESS_WEBSITE, "")))
        ...
        if (customerSettingModel.showOrderType) {
            getDineInOrderDetails?.orderTypeName?.trim()?.let {

                printerTasks.add(Pair(ORDER_TYPE, it))
            }
            printerTasks.add(Pair(BREAK_LINE, ""))
        }
...
//            }
            ...
            printerTasks.apply {
                add(
                    Pair(
                        RECEIPT_ID, "ReceiptID:" + if (getDineInOrderDetails?.offlineId?.isEmpty() == true) {
                            "ENTJKOIJH8745"
                        } else {
                            getDineInOrderDetails?.offlineId
                        }
            ...
            for (i in 0 until dineInList.size) {
                if (dineInList[i].isHeader == 0) {
                    ...
                    dineInList.get(i).item?.let {
                        addOrderItemForDineInInnerCommon(
                            printerTasks, it, customerSettingModel.fonts, customerSettingModel.showModifiers
                        )

//
//                    getPrinterTasks.forEach{
//                        printerTasks.add(Pair(it.first,it.second))
//                    }
...
//                        "-$" + MethodUtils.roundOffAmountString(0.0)
            ...
            var finalServiceCharges = getDineInOrderDetails?.totalServiceCharges ?: 0.0


            if (finalServiceCharges != 0.0) {
                val str4 = padLine(
                    "Service Charge", "$" + MethodUtils.roundOffAmountString(
                        finalServiceCharges
                    ), if (customerSettingModel.fonts == LARGE) {
...
//        val totalPayments = getDineInOrderDetails?.payments?.forEach{
//            it.amount++
//        }
...
//        ).toString()
            ...
            if (getDineInOrderDetails?.totalTips == 0.0) {
                ...
                if (customerSettingModel.showTipLineForCash) {
                    ...
                    if (customerSettingModel.fonts == LARGE) {
                        ...
                        printerTasks.add(Pair(PRINT_TIPS_CUSTOM, "Tip       _____________"))
                        ...
                        printerTasks.add(
                            Pair(
                                PRINT_TIPS_CUSTOM, "Tip                               _____________"
                            )
                        )
                        printerTasks.add(
                            Pair(
                                PRINT_TOTAL_CUSTOM, "Total                             _____________"
                            )
                        )
            ...
            if (customerSettingModel.showTipSuggestion) {
                ...
                if (tipsList.isNotEmpty()) {
                    addTipsListInnerCommon(
                        printerTasks, tipsList, receiptModel?.order?.totalAmount?.toDouble() ?: 0.0, customerSettingModel.fonts
                    )
                    printerTasks.add(Pair(BREAK_LINE, ""))
                }
            ...
            if (receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.paymentType?.lowercase() == "Card".lowercase()) {/*val str12 = padLine(
                    "",
                    receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.cardName.toString(),
                    if (customerSettingModel.fonts == LARGE) 23 else 48
                ).toString()
                ...
                var strCardType = receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.cardType.toString()
            ...
            if (customerSettingModel.fonts == LARGE) {
                ...
                printerTasks.add(
                    Pair(
                        PRINT_CUSTOMER_SIGNATURE, "Customer Signature           __________________"
                    )
                )
            }
            ...
            if (customerSettingModel.showQrCode) {
                ...
                printerTasks.add(
                    Pair(
                        PRINT_QR_CODE, receiptModel?.order?.digital_receipt_url.toString()
                    )
                )
            }
            ...
            if (receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.paymentType.equals(
                ...
                when (printerType) {
                    CommonPrinterTypes.SunmiInnerPrinter -> try {
                        SunmiPrinterApi.getInstance().sendRawData(aa)
                    } catch (e: java.lang.Exception) {
                        e.printStackTrace()
                    }
    ...
    private fun printDineInTable1Inner() = try {
        LogUtil.logE("printDineReciept", "Staring....")
        PrintSunmiUtils.fontSizeInner(customerSettingModel.fonts)
        SunmiPrintHelper.getInstance().initPrinter()
...
//        }
        ...
        SunmiPrintHelper.getInstance().lineWrap(1)
        val str5 = padLine(
            "Total Price", "$" + MethodUtils.roundOffAmountString(totalAmt + finalServiceCharges), if (customerSettingModel.fonts == LARGE) 23 else 48
        ).toString()
        PrintSunmiUtils.printBoldText(prefProvider.isOldSunmiFrameworkVersion(), str5)
        ...
        if (getDineInOrderDetails?.totalTips == 0.0) {
            ...
            if (customerSettingModel.showTipLineForCash) {
                ...
                if (customerSettingModel.fonts == LARGE) {
                    PrintSunmiUtils.printBoldText(
                        prefProvider.isOldSunmiFrameworkVersion(), "Tip       _____________"
                    )
                    PrintSunmiUtils.printBoldText(
                        prefProvider.isOldSunmiFrameworkVersion(), "Total     _____________"
                    )
                    SunmiPrintHelper.getInstance().lineWrap(1)
                } else {
                    PrintSunmiUtils.printBoldText(
        ...
        if (getDineInOrderDetails?.payments?.isNotEmpty() == true) {
            printPayment(
                prefProvider.isOldSunmiFrameworkVersion(), printerType = SUNMI_INNER_PRINTER, list = getDineInOrderDetails!!.payments
            )
        ...
        if (customerSettingModel.showTipSuggestion) {
            ...
            if (tipsList.isNotEmpty()) {
                addTipsListInner(
                    tipsList, receiptModel?.order?.totalAmount?.toDouble() ?: 0.0, customerSettingModel.fonts, prefProvider.isOldSunmiFrameworkVersion()
                )
                SunmiPrintHelper.getInstance().lineWrap(1)
            }
        ...
        if (receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.paymentType?.lowercase() == "Card".lowercase()) {/*val str12 = padLine(
            ...
            if (!str13.isNullOrBlank()) {
                PrintSunmiUtils.printNormalText(prefProvider.isOldSunmiFrameworkVersion(), str13)
            }
            val str14 = padLine(
                "",
                receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.cardNumber.toString(),
            ...
            if (!str14.isNullOrBlank()) {
                SunmiPrintHelper.getInstance().lineWrap(1)
                PrintSunmiUtils.printNormalText(prefProvider.isOldSunmiFrameworkVersion(), str14)
                SunmiPrintHelper.getInstance().lineWrap(1)
            }
        }
        ...
        SunmiPrintHelper.getInstance().lineWrap(2)
        PrintSunmiUtils.printNormalText(
            prefProvider.isOldSunmiFrameworkVersion(), "__________________________"
        )
        SunmiPrintHelper.getInstance().lineWrap(1)
            ...
            if (!isLastPayment) {
                ...
                if (findNavController().currentDestination?.id == R.id.orderCompleteFragment) {
                    findNavController().navigate(
                        R.id.action_orderCompleteFragment_to_dineInOrderTable, bundle
                    )
                }
...
//         Runtime.getRuntime().gc()
        ...
        if (!isSpilt) {
            prefProvider.setValue(Constants.OPEN_ORDER_ITEMS, "")/*Added By Rahul */
            prefProvider.setValue(Constants.OPEN_ORDER_ITEMS_OLD, "")
            ...
            if (isGuest) {
                if (!isLastPayment) {
                    ...
                    if (prefProvider.getValueboolean(LOCK_SCREEN_TRANSACTION, false)) {
                        ...
                        if (viewModelDashBoard.boldPosNeedToRefresh) findNavController().navigate(R.id.action_orderCompleteFragment_to_dashboardCategoryNew)
                        else findNavController().navigate(R.id.action_orderCompleteFragment_to_passcode)
    ...
    fun saveDataInPrefrences() {
        ...
        var model = SplitBundleModel(
            ...
            dineInList,
            prefProvider.getValue(SUB_TOTAL, "0.0").toDouble(),
            ...
            prefProvider.getValue(CASH_DISCOUNT_SURCHARGE, "0.0").toDouble(),
            cartList,
            redeemLoyaltyInfo
        )
    ...
    private fun getKitchenPrinters() {
        viewModel.getKitchenPrinterList().observe(viewLifecycleOwner, object : Observer<Resource<List<PrinterResponse.Data.KitchenReceiptPrinters>>> {
                                ...
                                if (!requireArguments().getBoolean("isSpilt")) {
                                    if (!requireArguments().getBoolean("isDineIn") && !requireArguments().getBoolean(
                                        ...
                                        if (receiptModel?.order?.orderType == OPEN_ORDER) {
                                            ...
                                            if (prefProvider.getValueboolean(
                                                    OPEN_ORDER_UPDATE_FOR_PRINT, false
                                                ) == true
                                            ) {
                                                    ...
                                                    if (kitchenPrinterList[i].kitchenStatus) {
                                                        kitchenPrinterList[i].orderTypes.forEach {
                                                            ...
                                                            if (it.orderTypeId == receiptModel?.order?.orderTypeId || it.orderType == receiptModel?.order?.orderType
                                                                ...
                                                                it.printerSettings.forEach {
                                                                    if ((it.printType.lowercase().equals(KITCHEN.lowercase()) || it.printType.lowercase()
                                                                            .equals(
                                                                        ...
                                                                        if (checkItemsforPrinter(
                                                                            ...
                                                                            if (!prefProvider.getValueboolean(
                                                                                ...
                                                                                if (prefProvider.getValueboolean(
                                                                                        ...
                                                                                        Constants.DO_PRINT_CUSTOM, false
                                                                                    )
                                                                                ) {
                                                                                    Log.e(TAG2, "initKit 9271")
                                                                                    initKitchenPrinter(kitchenPrinterList.get(i), KITCHEN)
                                                                                } else {
                                                                                    var oldCartModelString = prefProvider.getValue(
                                                                                        "BEFORE_ORDER_NOTE", ""
                                                                                    )
                                                                                    if (oldCartModelString.isNotEmpty()) {
                                                                                        var oldCartModel = Gson().fromJson<CartModel>(
                                                                                            oldCartModelString, CartModel::class.java
                                                                                        )
                                                                                        if (!oldCartModel.note.equals(
                                                                                                cartList?.note
                                                                                            )
                                                                                        ) {
                                                                                            Log.e(TAG2, "initKit 9285")
                                                                                            initKitchenPrinter(kitchenPrinterList.get(i), KITCHEN)
                                                                                        }
                                                                                    }
                                                                                }
                                                    ...
                                                    for (i in kitchenPrinterList.indices) {
                                                        if (kitchenPrinterList[i].kitchenStatus) {
                                                            kitchenPrinterList[i].orderTypes.forEach {
                                                                ...
                                                                if (it.orderTypeId == receiptModel?.order?.orderTypeId
                                                                    || it.orderTypeName == receiptModel?.order?.orderTypeName
                                                                ) {

                                                                    it.printerSettings.forEach {
                                                                        if (it.printType.lowercase().equals(KITCHEN.lowercase()) && it.autoPrinting) {
                                                                            ...
                                                                            if (checkItemsforPrinter(
                                                                                ...
                                                                                if (!prefProvider.getValueboolean(Constants.NO_NEED_TO_PRINT, false)) {
                                                                                    if (prefProvider.getValueboolean(Constants.DO_PRINT, false)) {
                                                                                        Log.e(TAG2, "initKit 9365: ${Gson().toJson(kitchenPrinterList[i])}")
                                                                                        initKitchenPrinter(kitchenPrinterList.get(i), KITCHEN)
                                                                                    } else if (prefProvider.getValueboolean(
                                                                                            Constants.DO_PRINT_CUSTOM, true
                                                                                        )
                                                                                    ) {
                                                                                        Log.e(TAG2, "initKit 9371")
                                                                                        initKitchenPrinter(kitchenPrinterList.get(i), KITCHEN)
                                                                                    } else {
                                                                                        try {
                                    ...
                                    Log.d("Kiosk", "Kiosk")/* if (prefProvider.getValue(Constants.ORDER_TYPE,"").equals(Constants.KIOSK_OPEN_ORDER)){
 //                                            Print the receipt here
                            ...
                            if (remainingAmount == 0.0) prefProvider.setValueboolean(Constants.DO_PRINT_CUSTOM, false)
...
//                                                                val kitchenPrinterList = kitchenPrinterList[i]
                                                            ...
                                                            Log.e(TAG2, "initKit 9455: ${Gson().toJson(kitchenPrinterList[i])}")
                                                            initKitchenPrinter(kitchenPrinterList[i], KITCHEN)
                                                        }
...
//                    ProgressUtils.showProgressDialog(requireActivity())
                    ...
                    Status.ERROR -> {
                        ...
                        EventBus.getDefault().post(
                            MessageEvent(
                                "${Constants.LINE_BREAK_TAB} OrderCompleteFragment.kt_getKitchenPrinters()_ERROR -> ${
                                    Gson().toJson(
                                        it
                                    )
                                }"
    ...
    private fun checkOrderItemsForOpenORderUpdate(): ArrayList<CreateOrderResponse.Data.Order.OrderItem> {
        var printOrderItems: ArrayList<CreateOrderResponse.Data.Order.OrderItem> = arrayListOf()
        ...
        val serializedObject: String = prefProvider.getValue(
        ...
        if (serializedObject.isNotEmpty()) {
            val gson = Gson()
            val type = object : TypeToken<List<CreateOrderResponse.Data.Order.OrderItem?>?>() {}.type
            var arrayItems: ArrayList<CreateOrderResponse.Data.Order.OrderItem> = gson.fromJson<Any>(
            ...
            Log.e(
                TAG, "updafwwewe  ${Gson().toJson(Gson().toJson(receiptModel?.order?.orderItems))}"
            )
                        ...
                        if (orderItemJ.itemId == orderItem.itemId) {
                            if (orderItemJ.quantity != orderItem.quantity) {
                                if (orderItem.quantity > orderItemJ.quantity) {
                                    orderItem.quantity = orderItem.quantity - orderItemJ.quantity
            ...
            when (it.status) {
                Status.SUCCESS -> {
                    ...
                    if (it.data != null && isPrintCustomer) {
                        ...
                        if (IS_GIFT_CARD_TYPE) {
                            ...
                            if (autoPrintCheck) {
                                        ...
                                        cus.orderTypes.forEach {
                                            ...
                                            if (it.orderTypeId == receiptModel?.order?.orderTypeId || it.orderType == receiptModel?.order?.orderType) {
                                                ...
                                                it.printerSettings.forEach {
                                                    ...
                                                    if (it.printType.lowercase().equals(CUSTOMER.lowercase()) && it.autoPrinting) {
                                ...
                                customerList.forEach {
                                    if (it.customerStatus) {
                                        initPrinter(
                                            it, CUSTOMER, autoPrintCheck, fromllPrintButton = true
                                        )
                                    }
                        ...
                        Log.d(TAG2, "Start printer getCustomerPrinters(autoPrint...)  else _1")
                        EventBus.getDefault().post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragment.getCustomerPrinters(autoPrint...)  else _1"))
...
//                    ProgressUtils.showProgressDialog(requireActivity())
    ...
    fun checkBluetoothPermissions(onBluetoothPermissionGranted: OnBluetoothPermissionGranted) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.S && ContextCompat.checkSelfPermission(
            ...
            ActivityCompat.requestPermissions(
                requireActivity(), arrayOf<String>(Manifest.permission.BLUETOOTH), Constants.PERMISSION_BLUETOOTH
            )
            ...
            ActivityCompat.requestPermissions(
                requireActivity(), arrayOf<String>(Manifest.permission.BLUETOOTH_ADMIN), Constants.PERMISSION_BLUETOOTH_ADMIN
            )
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S && ContextCompat.checkSelfPermission(
            ...
            ActivityCompat.requestPermissions(
                requireActivity(), arrayOf<String>(Manifest.permission.BLUETOOTH_CONNECT), Constants.PERMISSION_BLUETOOTH_CONNECT
            )
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S && ContextCompat.checkSelfPermission(
            ...
            ActivityCompat.requestPermissions(
                requireActivity(), arrayOf<String>(Manifest.permission.BLUETOOTH_SCAN), Constants.PERMISSION_BLUETOOTH_SCAN
            )
        } else {
    ...
    private fun initPrinter(
        customerReceiptPrinters: PrinterResponse.Data.CustomerReceiptPrinters, type: String, isAutoPrint: Boolean, fromllPrintButton: Boolean = false
    ) {
        Log.e(TAG, "checkAutoPrint  ${isAutoPrint}")
        pd?.show()
        Log.d(TAG2, "Start printer OrderCompleteFragment.kt _initPrinter(customerRecei...")
        EventBus.getDefault().post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragment.kt _initPrinter(customerRecei..."))
            ...
            if (customerReceiptPrinters.name.startsWith(SUNMI_PRINTER, true)) {
                ...
                if (PrinterClass.getPrinter() == null) {
                    ...
                    if (printer != null) {
                        ...
                        printer.setBatteryStatusChangeEventCallback(this@OrderCompleteFragment)
                    }
                    ...
                    try {
                        ...
                        printer?.openPrinter(
                            ...
                            if (customerReceiptPrinters.printer_type == BLUETOOTH) {
                                ...
                                Print.DEVTYPE_TCP
                            }, customerReceiptPrinters.ipAddress, enabled, 10000
                        )
                        //printer?.setStatusChangeEventCallback(this)
    ...
    private fun printFromLandiInnerPrinter(
        isAutoPrint: Boolean, customerReceiptPrinters: PrinterResponse.Data.CustomerReceiptPrinters
    ) {
            ...
            this.checkBluetoothPermissions(object : OnBluetoothPermissionGranted {
                override fun onPermissionsGranted() {
                    ...
                    try {
                        ...
                        EventBus.getDefault().post(
                            MessageEvent(
                                "${Constants.LINE_BREAK_TAB} OrderCompleteFragement.kt_printFromLandiInnerPrinter() = ${
                                    Gson().toJson(printer)
                                }"
                            )
                        )
                        ...
                        EventBus.getDefault()
                            .post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragement.kt_printFromLandiInnerPrinter()_Catch = ${ex.printStackTrace()}"))
                        requireActivity().runOnUiThread(Runnable {
                            Toast.makeText(
                                activity, "Printer is not available...", Toast.LENGTH_LONG
                            ).show()
                        })
                        return
                    }
                    GlobalScope.launch {
                        LPrint.connectLandiInnerPrinter(customerReceiptPrinters.macAddress)?.let { outputStream ->
                            /*---------------------Sample---------------------*//*   outputStream.write(LPrint.CENTER_ALIGN)  // Center align
...
//                            PrintSunmiUtils.printLogoInner(prefProvider.getValue(VENUE_LOGO, ""))
                                    ...
                                    printCenter(
                                        prefProvider.getValue(
                                            BUSINESS_NAME, ""
                                        ), fontSize = FONT_SIZE_5X, isBold = true, printOnNewLine = true
                                    )
//                                        lineBreak()
...
//                                        outputStream.write(it.toByteArray())
                                            ...
                                            printCenter(
                                                it, isBold = true, fontSize = FONT_SIZE_5X
                                            )
...
//                                            outputStream.write(LPrint.LINE_FEED)
                                    ...
                                    if (receiptModel?.order?.orderType.equals(PHONE_ORDER, true) || receiptModel?.order?.orderType.equals(
                                            "OnlineWebOrder", true
                                        ) || receiptModel?.order?.orderType.equals(
                                            "Online Order", true
...
//                                outputStream.write("ReceiptID: ${receiptModel?.order?.offlineId?.trim()}".toByteArray())
                                    ...
                                    receiptModel?.order?.orderItems?.let {
                                        addOrderItemsInnerNewLandi(
                                            it, customerSettingModel.showModifiers, customerSettingModel.fonts, LPrint
                                        )
                                    }
                                    lineBreak()
...
//                            "-$" + MethodUtils.roundOffAmountString(0.00)
                                    ...
                                    if (order?.totalTaxAmount != null) {
                                        ...
                                        val taxToPrint = padLine(
                                            "Tax", "$" + MethodUtils.roundOffAmountString(order.totalTaxAmount), if (customerSettingModel.fonts == LARGE) {
                                                23
                                            } else {
                                                48
                                            }
                                    ...
                                    if (serviceCharge != null && (order?.serviceChargeEnabled == true)) {

                                        val serviceChargeToPrint = padLine(
                                            "Service Charge",
                                            "$" + MethodUtils.roundOffAmountString(order.totalServiceCharges),
                                            if (customerSettingModel.fonts == LARGE) {
                                                23
                                            } else {
                                                48
                                    ...
                                    if (tipBefore == 0.0) order?.payments?.last()?.tips = tipAfterAmount

                                    if (order?.payments?.last()?.tips != 0.0) {


                                        val tipsToPrint = padLine(
                                            "Tip", "$" + order?.payments?.last()?.tips?.let {
                                                MethodUtils.roundOffAmountString(
                                                    it + if (order.cash_discount_type.lowercase() == "cashdiscount" && order.payments.last().paymentType.lowercase(
                                                            Locale.getDefault()
                                                        ) == "card"
                                                    ) order.payments.last().cash_discount_or_surcharge else 0.0
                                                )
                                            }, 48
                                        ).toString()
                                        ...
                                        val tipsToPrint = padLine(
                                            "Tip", "$" + MethodUtils.roundOffAmountString(tipAfterAmount), 48
                                        ).toString()
                                        printLeft(tipsToPrint)
                                        lineBreak()
                                    }
                                    ...
                                    /**
                                     * Print cash discount or surcharge details
                                     */
                                    ...
                                    if (customerSettingModel.showCashDisSurCharg) {
                                        if (receiptModel?.order?.payments?.isNotEmpty() == true && receiptModel?.order?.payments?.get(
                                            ...
                                            if (receiptModel?.order?.totalCashDiscountFee != null) {
                                                ...
                                                val str8 = padLine(
                                                    Constants.SURCHARGE_TEXT, "$" + MethodUtils.roundOffAmountString(
                                                        receiptModel?.order?.payments?.get(
                                                ...
                                                getString(R.string.external), ignoreCase = true
                                            )
                                        ) {
                                            ...
                                            if (receiptModel?.order?.totalCashDiscountFee != null) {
                                                ...
                                                val str8 = padLine(
                                                    "Cash Discount", if (receiptModel?.order?.totalCashDiscountFee == 0.0) {
                                                        "$" + MethodUtils.roundOffAmountString(
                                                            receiptModel?.order?.payments?.get(
                                                                receiptModel?.order?.payments?.size?.minus(
                                                                    1
                                    ...
                                    if (receiptModel?.order?.payments?.isNotEmpty() == true) {
                                        if (receiptModel?.order?.payments?.get(0)?.isLoyaltyApplied == true && receiptModel?.order?.payments!![0].loyaltyUSedPoints != 0) {
                                            if (receiptModel?.order?.loyaltyAmount != 0.0) {
                                                ...
                                                val loyaltyAmt = padLine(
                                                    "Used Loyalty Amount", "-$" + receiptModel?.order?.loyaltyAmount?.let {
                                                        MethodUtils.roundOffAmountString(
                                                            it
                                                        )
                                                    }, 48
                                                ...
                                                printLeft(loyaltyAmt)

                                            }


                                            val loyaltyPnt = padLine(
                                                "Used Loyalty Points",
                                                receiptModel?.order?.payments!![0].loyaltyUSedPoints.toString(),
                                                if (customerSettingModel.fonts == LARGE) 23 else 48
                                            ).toString()

                                            printLeft(loyaltyPnt)
                                            lineBreak()
                                    ...
                                    val totalAmt = receiptModel?.order?.payments?.last()?.amount?.plus(
                                        receiptModel?.order?.payments?.last()?.tips!!
                                    )
//                                            MethodUtils.roundOffAmountDouble(calculateTotal)


                                    val totalAmountToPrint = padLine(
                                        "Total Price", "$" + totalAmt?.let {
                                            MethodUtils.roundOffAmountString(
                                                it
                                            )
                                        }, 48
                                ...
                                lineBreak()*/
                                    ...
                                    if (order!!.payments.last().paymentType == "Cash") {
                                        ...
                                        val changeAmount =
                                            paidAmount - receiptModel?.order?.payments?.last()?.amount?.plus(receiptModel?.order?.payments?.last()?.tips!!)!!
...
//                                                ).toString()
                                    ...
                                    if (isSpilt) {
                                        ...
                                        val remainAmt = padLine(
                                            "Remaining Amount", "$" + MethodUtils.roundOffAmountString(
                                                remainingAmount
                                            ), if (customerSettingModel.fonts == LARGE) 23 else 48
                                        ).toString()

                                        printBoldLeft(remainAmt)
                                        lineBreak()
                                    }
                                        ...
                                        if (tipsList.isNotEmpty()) {
                                            val tipsToPrint = totalAmt?.let {
                                                ...
                                                addTipsListInnerLandi(
                                                    tipsList, MethodUtils.roundOffAmountDouble(
                                                        it
                                                    ), customerSettingModel.fonts
                                                )
                                            }
                                    ...
                                    if (receiptModel?.order?.payments?.isNotEmpty() == true) {
                                        ...
                                        try {/*receiptModel?.order?.payments?.last()?.id*/
                                            val str10 = padLine(
                                                "Transaction ID", "" + receiptModel?.order?.payments?.size?.minus(
                                                    1
                                                )?.let {
                                                    receiptModel?.order?.payments?.get(
                                                        it
                                            ...
                                            val str11 = padLine(
                                                "Transaction Type", receiptModel?.order?.payments?.get(
                                                    receiptModel?.order?.payments?.size!! - 1
                                                )?.paymentType, 48
                                            ).toString()
                                            printLeft(str11)
...
//                                                receiptModel?.order?.payments?.last()?.cardType ?: ""
                                        ...
                                        val strCardNumber = receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.cardNumber.toString()
...
//                                                receiptModel?.order?.payments?.last()?.cardNumber
                                    ...
                                    if (customerSettingModel.showCustomerAddress or customerSettingModel.showCustomerPhone or customerSettingModel.showCustomerName) {
                                        ...
                                        if (order.customer != null) {
                                            ...
                                            PrintSunmiUtils.customerDetailsInnerLandi(
                                                false, LPrint
                                            )

                                            if (customerSettingModel.showCustomerName) {

                                                printLeft(order.customer.firstName + " " + order.customer.lastName)
                                                lineBreak()
                                            }
                                            ...
                                            if (customerSettingModel.showCustomerAddress) {
                                                if (order.customer.addresses.isNotEmpty() == true) {


                                                    order.customer.addresses.filter { it.typeOfAddress == SHIPPING_ADDRESS }.forEach {

                                                        if (it.typeOfAddress.equals(
                                                                SHIPPING_ADDRESS, ignoreCase = true
                                                            )
                                                        ) {
                                                            printLeft(
                                                                it.fullAddress
...
//                                        printCenter()
                                    ...
                                    if (customerSettingModel.showQrCode) {
                                        printQRCode(
                                            outputStream, order.digital_receipt_url.toString(), CENTER_ALIGN
                                        )
                                    }
...
//                                LPrint.disconnectLandiPrinter()
                                        ...
                                        if (woyouService != null) {
                                            try {
                                                woyouService!!.sendRAWData(
                                                    byteArrayOf(0x1B, 0x45, 0x01), this@OrderCompleteFragment
                                                )
                                            } catch (e: Exception) {
                                                e.printStackTrace()
                                            }
    ...
    private fun generatePrint(
        customerReceiptPrinters: PrinterResponse.Data.CustomerReceiptPrinters, type: String, isAutoPrint: Boolean
    ) {
        var builder: Builder? = null
        try {
            builder = Builder(
            ...
            if (receiptModel?.order?.orderType.equals(PHONE_ORDER, true) || receiptModel?.order?.orderType.equals(
                    "OnlineWebOrder", true
                ) || receiptModel?.order?.orderType.equals("Online Order", true) || receiptModel?.order?.orderType.equals("OnlineOrder", true)
            ) {
            ...
            if (customerSettingModel.fonts == LARGE) {
                ...
                builder.addText(
                    padLine(
                        "ReceiptID:" + receiptModel?.order?.offlineId, "", if (customerSettingModel.fonts == LARGE) {
                            24
                        } else {
                            48
                        }
...
//                            "-$" + MethodUtils.roundOffAmountString(receiptModel?.order?.totalDiscount!!)
            ...
            builder.addText(
                padLine(
                    "Sub Total", "$" + MethodUtils.roundOffAmountString(receiptModel?.order?.subTotal!!), if (customerSettingModel.fonts == LARGE) {
                        24
                    } else {
                        48
                    }
            ...
            if (receiptModel?.order?.totalTaxAmount != null) {
                ...
                builder.addText(
                    padLine(
                        "Tax", "$" + MethodUtils.roundOffAmountString(receiptModel?.order?.totalTaxAmount!!), if (customerSettingModel.fonts == LARGE) {
                            24
                        } else {
                            48
                        }
            ...
            if (tipAmount > 0) {
                ...
                builder.addText(
                    padLine(
                        "Tip", "$" + MethodUtils.roundOffAmountString(
                            tipAmount.toDouble() + if (order?.cash_discount_type?.lowercase() == "cashdiscount" && order.payments.last().paymentType.lowercase(
                                    Locale.getDefault()
                                ) == "card"
                            ) order.payments.last().cash_discount_or_surcharge else 0.0
                        ), if (customerSettingModel.fonts == LARGE) {
            ...
            if (receiptModel?.order?.payments?.isNotEmpty() == true) {
                if (receiptModel?.order?.payments?.get(0)?.isLoyaltyApplied == true && receiptModel?.order?.payments!![0].loyaltyUSedPoints != 0) {
                    ...
                    builder.addText(
                        padLine(
                            "Used Loyalty Points", receiptModel?.order?.payments!![0].loyaltyUSedPoints.toString(), if (customerSettingModel.fonts == LARGE) {
                                24
                            } else {
                                48
                            }
            ...
            if (receiptModel?.order?.totalAmount != null) {
                ...
                if (paymentType == "Cash") {
                    ...
                    var finalAmt: Double = (receiptModel?.order?.subTotal ?: 0.0).plus(receiptModel?.order?.totalTaxAmount ?: 0.0)
                    ...
                    if (receiptModel?.order?.cash_discount_type?.lowercase() == "CashDiscount".lowercase()) {
                        ...
                        builder.addText(
                            padLine(
                                "Total Price", "$" + MethodUtils.roundOffAmountString(finalAmt), if (customerSettingModel.fonts == LARGE) {
                                    24
                                } else {
                                    48
                                }
            ...
            if (customerSettingModel.showCustomerAddress || customerSettingModel.showCustomerPhone || customerSettingModel.showCustomerName) {
                ...
                if (receiptModel?.order?.customer != null && !prefProvider.getValue(
                    ...
                    builder.addText(
                        padLine(
                            "Customer Details", "", if (customerSettingModel.fonts == LARGE) {
                                24
                            } else {
                                48
                            }
                    ...
                    if (customerSettingModel.showCustomerPhone) {
                        ...
                        if (receiptModel?.order?.customer?.phones?.isNotEmpty() == true) {
                            ...
                            if (phoneNoLast.isNotEmpty()) {
                                ...
                                builder.addText(
                                    padLine(
                                        MethodUtils.formatPhoneNumber(phone.toString()), "", if (customerSettingModel.fonts == LARGE) {
                                            24
                                        } else {
                                            48
                                        }
                    ...
                    if (customerSettingModel.showCustomerAddress && !prefProvider.getValue(
                        ...
                        if (receiptModel?.order?.customer?.addresses?.isNotEmpty() == true) {
                            ...
                            receiptModel?.order?.customer?.addresses?.filter { it.typeOfAddress == SHIPPING_ADDRESS }?.forEach {
                                ...
                                if (it.typeOfAddress.equals(
                                    ...
                                    builder.addText(
                                        padLine(
                                            it.fullAddress, "", if (customerSettingModel.fonts == LARGE) {
                                                24
                                            } else {
                                                48
                                            }
            ...
            try {
                if (receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.paymentType.equals(
                    ...
                    builder.addPulse(
                        Printer.DRAWER_HIGH, Printer.PULSE_100
                    )
                }
                PrinterClass.getPrinter()?.sendData(
                    builder, BLUETOOTH_TIMEOUT, status, battery
    ...
    private fun generatePrintForGiftCard(
        customerReceiptPrinters: PrinterResponse.Data.CustomerReceiptPrinters, type: String
    ) {
        var builder: Builder? = null
        try {
            builder = Builder(
            ...
            if (customerSettingModel.fonts == LARGE) {
                ...
                if (customerSettingModel.showOrderTime) {
                    ...
                    builder.addText(
                        "Order Time:" + getReceiptFormatDateFromUTCServer(
                            requireContext(), giftCardReceiptModel?.gift_card?.payments?.get(
                                giftCardReceiptModel?.gift_card?.payments?.size?.minus(1) ?: 0
                            )?.created_at!!
                        )
                    )
                ...
                builder.addText(
                    padLine(/*"ReceiptID:" + giftCardReceiptModel?.gift_card?.id,
                        ""*/
                        "ReceiptID:" + giftCardReceiptModel?.gift_card?.payments?.get(
                            giftCardReceiptModel?.gift_card?.payments?.size?.minus(1) ?: 0
                        )?.offline_id, "", if (customerSettingModel.fonts == LARGE) {
                            24
                        } else {
                            48
                        }
            ...
            if (giftCardReceiptModel?.gift_card?.payments != null && giftCardReceiptModel?.gift_card?.payments?.isNotEmpty()!!) {
                giftCardAmount = giftCardReceiptModel?.gift_card?.payments?.get(giftCardReceiptModel?.gift_card?.payments?.size!! - 1)?.amount?.toPrecision(
                    2
                )?.toDoubleWithPrecision(2)!!
            }

            Log.d(
                TAG, "generatePrintForGiftCard:  giftCardAmount = ${giftCardAmount.toPrecision(2)}"
            )

            giftCardList.add(
                0, CreateOrderResponse.Data.Order.OrderItem(
                    itemName = "${giftCardReceiptModel?.gift_card?.gift_card_type} Gift Card", price = giftCardAmount, quantity = 1
                )
            )

            addOrderItems(
                builder, giftCardList, customerSettingModel.fonts, customerSettingModel.showModifiers
            )
            ...
            builder.addText(
                padLine(
                    "Total Price", "$${giftCardAmount.toPrecision(2)}", if (customerSettingModel.fonts == LARGE) {
                        24
                    } else {
                        48
                    }
            ...
            builder.addText(
                padLine(
                    "Transaction ID",
                    "" + giftCardReceiptModel?.gift_card?.payments?.size?.minus(1)?.let { giftCardReceiptModel?.gift_card?.payments?.get(it)?.id },
                    if (customerSettingModel.fonts == LARGE) {
                        24
                    } else {
                        48
                ...
                if (giftCardReceiptModel?.gift_card?.customer != null) {
                    ...
                    if (customerSettingModel.showCustomerPhone) {
                        ...
                        if (giftCardReceiptModel?.gift_card?.customer?.phones?.isNotEmpty() == true) {
                            ...
                            var phoneNoLast = giftCardReceiptModel?.gift_card?.customer?.phones?.get(
                            ...
                            if (phoneNoLast.isNotEmpty()) {
                                ...
                                val phone = giftCardReceiptModel?.gift_card?.customer?.phones?.size?.minus(
                                    1
                                )?.let {
                    ...
                    if (customerSettingModel.showCustomerAddress && !prefProvider.getValue(
                        ...
                        if (giftCardReceiptModel?.gift_card?.customer?.addresses?.isNotEmpty() == true) {
                            ...
                            giftCardReceiptModel?.gift_card?.customer?.addresses?.filter { it.typeOfAddress == SHIPPING_ADDRESS }?.forEach {
    ...
    private fun initKitchenPrinter(data: PrinterResponse.Data.KitchenReceiptPrinters, type: String) {
        Log.d(TAG2, "initKitchenPrinter data: ${Gson().toJson(data)}")
        Handler(Looper.getMainLooper()).postDelayed({
            prefProvider.deleteValue(Constants.DO_PRINT)
                ...
                GlobalScope.launch(Dispatchers.IO) {
                    mutex.withLock {
                        if (data.name.startsWith(SUNMI_PRINTER, true)) {
                            ...
                            try {
                                Log.d(
                                    "initKitchenPrinter", "SunmiBlueToothPrinter is ${data.ipAddress}"
                                )
...
//                    Log.d("KioskOpenOrderKitchenPrint", "CloudNotConnected")
                                generateKitchenReceiptSunmi(data, type)/* viewLifecycleOwner.lifecycleScope.launch {
                         delay(200)
...
//                Log.d("KioskOpenOrderKitchenPrint", "SunmiInnerSetService2")
                            ...
                            withContext(Dispatchers.Main) {
                                ...
                                val settings = StarConnectionSettings(InterfaceType.Lan, data.macAddress)
                                val printer = StarPrinter(settings, requireContext())

                                try {
                                    Log.d(TAG2, "Start printer start")
//                                    var printer: StarPrinter/*Star label printer - END*/
                                    Log.d(TAG2, "Start printer OrderCompleteFragment_TSP_printing_start")
                                    EventBus.getDefault().post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragment_TSP_printing_start"))

                                    val builder = StarXpandCommandBuilder()

                                    val printerBuilder = PrinterBuilder()

                                    Log.d(TAG2, "Start printer setup TSP_printing GlobalScope")
                                    EventBus.getDefault().post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragment_TSP_printing GlobalScope"))
                                    Log.d(TAG2, "Start printer orderItems: ${Gson().toJson(receiptModel?.order?.orderItems)}")
                                    ...
                                    with(printerBuilder) {
                                        ...
                                        Log.d(TAG, "initKitchenPrinter !oneItemPerReceipt: ${!oneItemPerReceipt}")
                                        if (!oneItemPerReceipt) {
                                            Log.d(TAG2, "Start printer TSP_printing_oneItemPerReceipt")
                                            EventBus.getDefault()
                                                .post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragment_TSP_printing_oneItemPerReceipt -> ${oneItemPerReceipt}"))

                                            var counter = 1
                                            var totalQuantity = 0

                                            receiptModel?.order?.orderItems?.forEach { item ->
                                                Log.d(TAG2, "item1: $item")
                                            ...
                                            receiptModel?.order?.orderItems?.forEach { item ->
                                                Log.d(TAG2, "item2: $item")
                                                if (!item.isPrinted || item.isItemEdited ||
                                                    (orderTypeToCheckKiosk.contentEquals("KioskOpenorder", true) && !item.isItemEdited)) {
                                                    // In case of Single item per receipt, isPrinted variable is maintained
                                                    Log.d(TAG2, "Start printer TSP_printing_condition")
                                                    EventBus.getDefault()
                                                        .post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragment_TSP_printing_condition-> ${item}"))
                                                    data.printerCategories.forEach { category ->
                                                        if (category.id == item.categoryId && category.printerEnable) {
                                                            for (singularity in 1..item.quantity) {
                                                                ...
                                                                if (printOrderIDInStickyPrinter) {
                                                                    ...
                                                                    add(
                                                                        PrinterBuilder().styleBold(true).styleMagnification(
                                                                            MagnificationParameter(2, 2)
                                                                        ).actionPrintText("${counter++}/$totalQuantity")
                                                                    )
                                                                    actionFeedLine(2)
                                                                    add(
                                                                        PrinterBuilder().styleBold(true).styleMagnification(
                                                                ...
                                                                add(
                                                                    PrinterBuilder().styleAlignment(Alignment.Left).styleMagnification(
                                                                        MagnificationParameter(2, 2)
                                                                    ).actionPrintText(
                                                                        content = addOrderSingleItemForStarKitchen(
                                                                            1, item, data.printerCategories.toCollection(
                                                                                arrayListOf()
                                                                            )
                                                                        )
                                                                    )
                                                                ...
                                                                if (receiptModel?.order?.note?.isNotEmpty() == true && kitchenSettingModel.showOrderNote == true) {
                                                                    add(
                                                                        PrinterBuilder().styleAlignment(Alignment.Center).styleBold(true).actionPrintText(
                                                                ...
                                                                if (receiptModel?.order?.note?.isNotEmpty() == true && kitchenSettingModel.showOrderNote == true) {
                                                                    add(
                                                                        PrinterBuilder().styleAlignment(Alignment.Center).actionPrintText(
                                                                            content = if (receiptModel?.order?.note?.isNotEmpty() == true && kitchenSettingModel.showOrderNote == true) {
                                                                                    ...
                                                                                    add(
                                                                                        PrinterBuilder().styleAlignment(
                                                                                            ...
                                                                                            content = if (kitchenSettingModel.showCustomerPhone && receiptModel?.order?.customer?.phones?.get(
                                                                                                ...
                                                                                                var phoneNumber =
                                                                                                    receiptModel?.order?.customer?.phones?.get(0)?.phoneNumber.toString()
                                            ...
                                            val paidList = if (orderTypeToCheckKiosk.contentEquals("KioskOpenorder", true)) {
                                                receiptModel?.order?.orderItems?.filterNot { it.isPaid }
                                            } else {
                                                receiptModel?.order?.orderItems?.filter { it.isPaid }
                                            }
                                            Log.d(TAG2, "Start printer paidList: $paidList")
                                            Log.d(TAG2, "Start printer TSP_printing_2")
                                            EventBus.getDefault()
                                                .post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragment_TSP_printing_2 -> ${Gson().toJson(paidList)}"))
                                            if (paidList?.isNotEmpty() == true) {
                                                Log.d(TAG2, "Start printer TSP_printing_3")
                                                EventBus.getDefault().post(
                                                    MessageEvent(
                                                ...
                                                if (isOrderUpdated) {
                                                    add(
                                                        PrinterBuilder().styleMagnification(
                                                            MagnificationParameter(2, 2)
                                                        ).styleBold(true).actionPrintText(
                                                            "***** UPDATED *****"
                                                        )
                                                    )
                                                }
                                                ...
                                                add(
                                                    PrinterBuilder().styleMagnification(
                                                        ...
                                                        if (kitchenSettingModel.showOrderType) receiptModel!!.order.orderTypeName
                                                        else ""
                                                    )
                                                )
                                                ...
                                                add(
                                                    PrinterBuilder().styleBold(true).actionPrintText(
//                                                    "------------------------"
                                                        "--------------------------------------------"
                                                    )
                                                )
                                                ...
                                                add(
                                                    PrinterBuilder().styleAlignment(Alignment.Left).styleMagnification(
                                                        ...
                                                        content = addOrdersForStarKitchen(
                                                            paidList, data.printerCategories.toCollection(
                                                                arrayListOf()
                                                            )
                                                        )
                                                    )
                                                ...
                                                if (receiptModel?.order?.note?.isNotEmpty() == true && kitchenSettingModel.showOrderNote == true) {
                                                    ...
                                                    add(
                                                        PrinterBuilder().styleMagnification(
                                                            MagnificationParameter(2, 2)
                                                        ).styleAlignment(Alignment.Center).styleBold(true).actionPrintText(
                                                            content = if (receiptModel?.order?.note?.isNotEmpty() == true && kitchenSettingModel.showOrderNote == true) {
                                                                "\nOrder Note"
                                                            } else ""
                                                        )
                                                ...
                                                if (receiptModel?.order?.note?.isNotEmpty() == true && kitchenSettingModel.showOrderNote == true) {
                                                    add(
                                                        PrinterBuilder().styleMagnification(
                                                            MagnificationParameter(2, 2)
                                                        ).styleAlignment(Alignment.Center).actionPrintText(
                                                            content = if (receiptModel?.order?.note?.isNotEmpty() == true && kitchenSettingModel.showOrderNote == true) {
                                                ...
                                                try {
                                                    ...
                                                    Log.d(TAG2, "Start printer exception_1 ")
                                                    EventBus.getDefault().post(
                                                        MessageEvent(
                                                ...
                                                try {
                                                    ...
                                                    Log.d(TAG2, "Start printer exception_2 ")
                                                    EventBus.getDefault().post(
                                                        MessageEvent(
                                                ...
                                                try {
                                                        ...
                                                        add(
                                                            PrinterBuilder().styleMagnification(
                                                                ...
                                                                content = if (kitchenSettingModel.showCustomerPhone && receiptModel?.order?.customer?.phones?.get(
                                                                    ...
                                                                    val areaCode = phoneNumber.substring(
                                                                        0, 3
                                                                    )
                                                                    val firstPart = phoneNumber.substring(
                                                                        3, 6
                                                                    )
                                                                    val secondPart = phoneNumber.substring(6)
                                                    ...
                                                    Log.d(TAG2, "Start printer exception_3 ")
                                                    EventBus.getDefault().post(
                                                        MessageEvent(
                                                ...
                                                try {
                                                        ...
                                                        add(
                                                            PrinterBuilder().styleMagnification(
                                                                ...
                                                                content = if (kitchenSettingModel.showCustomerAddress && receiptModel?.order?.customer?.addresses?.get(
                                                                    ...
                                                                    var address = receiptModel?.order?.customer?.addresses?.get(
                                                                        0
                                                                    )?.fullAddress
                                                    ...
                                                    Log.d(TAG2, "Start printer exception_4 ")
                                                    EventBus.getDefault().post(
                                                        MessageEvent(
                                                            "${Constants.LINE_BREAK_TAB} OrderCompleteFragment_TSP_printing exception_3 -> ${e}"
                                                        )
                                                ...
                                                val printedList = if (orderTypeToCheckKiosk.contentEquals("KioskOpenorder", true)) {
                                                    receiptModel?.order?.orderItems?.filterNot { it.isPrinted }
                                                } else {
                                                    receiptModel?.order?.orderItems?.filter { it.isPrinted }
                                                }

                                                Log.d(TAG2, "Start printer printedList: $printedList")
                                                Log.d(TAG2, "Start printer TSP_printing_else-13585")
                                                EventBus.getDefault().post(
                                                    MessageEvent(
                                                ...
                                                if (printedList?.isNotEmpty() == true) {
                                                    Log.d(TAG2, "Start printer printedList?.isNotEmpty()?:false")
                                                    EventBus.getDefault().post(
                                                        MessageEvent(
...
//                                                    "------------------------"
                                                    ...
                                                    add(
                                                        PrinterBuilder().styleAlignment(Alignment.Left).styleMagnification(
                                                            ...
                                                            content = addOrdersForStarKitchen(
                                                                printedList, data.printerCategories.toCollection(
                                                                    arrayListOf()
                                                                )
                                                            )
                                                        )
                                                    ...
                                                    try {
                                                        ...
                                                        Log.d(TAG2, "Start printer exception_6")
                                                        EventBus.getDefault().post(
                                                            MessageEvent(
                                                    ...
                                                    try {
                                                        ...
                                                        Log.d(TAG2, "Start printer exception_7")
                                                        EventBus.getDefault().post(
                                                            MessageEvent(
                                                    ...
                                                    try {
                                                            ...
                                                            add(
                                                                PrinterBuilder().styleMagnification(
                                                                    ...
                                                                    content = if (kitchenSettingModel.showCustomerPhone && receiptModel?.order?.customer?.phones?.get(
                                                                        ...
                                                                        val secondPart = phoneNumber.substring(
                                                                            6
                                                                        )
                                                        ...
                                                        Log.d(TAG2, "Start printer exception_8")
                                                        EventBus.getDefault().post(
                                                            MessageEvent(
                                                    ...
                                                    try {
                                                        ...
                                                        Log.d(TAG2, "Start printer exception_9")
                                                        EventBus.getDefault().post(
                                                            MessageEvent(
                                                                "${Constants.LINE_BREAK_TAB} OrderCompleteFragment_TSP_printing exception_3 -> ${e}"
                                                            )
                                                    ...
                                                    Log.d(TAG2, "Not Printed: printedList is empty")
                                                }
                                            }
                                        }
                                    }

                                    Log.d(TAG2, "Start printer prepared")
                                    Log.d(TAG2, "Start printer TSP_printing_ document prepared")
                                    EventBus.getDefault()
                                        .post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragment_TSP_printing_ document prepared"))

                                    val document = DocumentBuilder().addPrinter(printerBuilder)
                                    ...
                                    Log.d(TAG2, "Start printer TSP_printing_ success")
                                    EventBus.getDefault().post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragment_TSP_printing_ success"))
                                    Log.d(TAG2, "Success")
                                } catch (e: Exception) {
                                    Log.d(TAG2, "Start printer TSP_printing exception")
                                    EventBus.getDefault().post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragment_TSP_printing exception -> ${e}"))
                                    Log.d(TAG2, "Error: ${e}")
                                } finally {
                                    Log.d(TAG2, "finally")
                                    Log.d(TAG2, "Start printer TSP_printing_ FINALIZED")
                                    printer.closeAsync().await()
                                    EventBus.getDefault().post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragment_TSP_printing_ FINALIZED"))
                                }
                            }
                        } else if (data.name.contains(LANDI_INNER_PRINTER, true)) {/*lifecycleScope.launch {
                                while (!isAdded) {
                            ...
                            if (isNotPrinted) {
                                ...
                                if (!data.name.substring(0, 6).toString().lowercase().contains("TM-m".lowercase())) {
                                    ...
                                    var mPrinter = if (data.name.substring(0, 6).toString().lowercase().contains("TM-m".lowercase())) {
                                        Log.e(TAG, "YesContains")
                                        Printer(
                                            Printer.TM_M30, Printer.MODEL_ANK, requireContext()
                                        )
                                    } else {
                                        Printer(
                                            Printer.TM_U220, Printer.MODEL_ANK, requireContext()
                                        )
                                    ...
                                    mPrinter.setReceiveEventListener { printer, i, printerStatusInfo, s ->
                                        ...
                                        if (printerStatusInfo.paperTakenSensor == Printer.REMOVAL_DETECT_PAPER) {
                                            Log.e(
                                                "PrinterEvent", "REMOVAL_DETECT_PAPER"
                                            )
                                        }

                                        if (printerStatusInfo.paperTakenSensor == Printer.REMOVAL_DETECT_UNKNOWN) {
                                    ...
                                    try {
                                        ...
                                        var printerAdd = if (data.printer_type == BLUETOOTH) "BT:" + data.macAddress else "TCP:" + data.ipAddress
                                    ...
                                    if (PrinterClass.getPrinter() == null) {
                                        ...
                                        var printer: Print? = Print(requireContext())/*if (printer != null) {
                           printer.setStatusChangeEventCallback(this)
                           printer.setBatteryStatusChangeEventCallback(this)
                       }*/
...
//                                    SunmiPrintHelper.getInstance().initPrinter()
                                    ...
                                    try {
                                        if (isOrderUpdated == true || cartList!!.isEdited == true) {
                                            printCenter(
                                                "***** UPDATED *****", isBold = true, fontSize = FONT_SIZE_5X
                                            )
//                                            PrintSunmiUtils.headerText("***** UPDATED *****")
                                    ...
                                    try {
                                        ...
                                        if (prefProvider.getValueboolean(
                                            ...
                                            printCenter(
                                                "OrderID:" + receiptModel?.order?.custom_order_id, isBold = true, fontSize = FONT_SIZE_5X
                                            )
                                        } else {
                                            printCenter(
                                                "OrderID:" + receiptModel?.order?.id, isBold = true, fontSize = FONT_SIZE_5X
                                            )
                                        }
                                        lineBreak()
                                    } catch (e: Exception) {
                                    ...
                                    if (kitchenSettingModel.showOrderType) {
                                        printCenter(
                                            receiptModel?.order?.orderTypeName.toString(), isBold = true, fontSize = FONT_SIZE_5X
                                        )
                                    }
                                    lineBreak()
                                    ...
                                    if (receiptModel?.order?.orderType.equals(
                                        ...
                                        printCenter(
                                            receiptModel?.order?.deliveryType.toString(), isBold = true, fontSize = FONT_SIZE_5X
                                        )
                                        lineBreak()
                                    }
                                    lineBreak()
                                    ...
                                    if (kitchenSettingModel.showTeamMember) {
                                        printLeft(
                                            "Employee:" + receiptModel?.order?.employee?.name, isBold = true, fontSize = FONT_SIZE_5X
                                        )
                                    }
                                    lineBreak()
...
//                                    }
                                    ...
                                    receiptModel?.order?.orderItems?.let {
                                        ...
                                        addOrdersForKitchenLandiInner(
                                            it, kitchenReceiptPrinters.printerCategories.toCollection(
                                                arrayListOf()
                                            ), lprint = LPrint
                                        )
                                    }
                                    ...
                                    if (receiptModel?.order?.note?.isNotEmpty() == true && kitchenSettingModel.showOrderNote) {
                                        ...
                                        lineBreak()
                                        printCenter(
                                            "Order Note", isBold = true, fontSize = FONT_SIZE_5X
                                        )
                                        lineBreak()
                                        printCenter(
                                            receiptModel?.order?.note.toString(), isBold = false, fontSize = FONT_SIZE_5X
                                        )
                                        lineBreak()
                                    }
                                    ...
                                    if (kitchenSettingModel.showCustomerAddress != false || kitchenSettingModel.showCustomerPhone != false || kitchenSettingModel.showCustomerName != false) {
                                        if (receiptModel?.order?.customer != null) {
                                            ...
                                            lineBreak()
                                            printLeft(
                                                "Customer Details", isBold = true, fontSize = FONT_SIZE_5X
                                            )
                                            lineBreak()
                                            printDashedLineAndBreak()
                                        ...
                                        if (sunmiFrameworkVersion?.get(0)
                                                    ...
                                                    if (receiptModel?.order?.customer?.phones?.isNotEmpty() == true) {
                                                        ...
                                                        receiptModel?.order?.customer?.phones?.get(
                                                            ...
                                                            printLeft(
                                                                MethodUtils.formatPhoneNumber(
                                                                    it
                                                                ), isBold = false, fontSize = FONT_SIZE_5X
                                                            )
                                                        }
                                                    }
                                            ...
                                            if (kitchenSettingModel.showCustomerAddress) {
                                                if (receiptModel?.order?.orderType?.trim().toString().lowercase() == "Open Order".trim().toString()
                                                        .lowercase() && receiptModel?.order?.deliveryType?.trim().toString().lowercase() == "Pickup".trim()
                                                        .lowercase()
                                                ) {
                                                    ...
                                                    try {
                                                        if (receiptModel?.order?.customer?.addresses?.isNotEmpty() == true) {
                                                            ...
                                                            receiptModel?.order?.customer?.addresses?.get(0)?.fullAddress?.let {
                                                                printLeft(
                                                                    it, isBold = false, fontSize = FONT_SIZE_5X
                                                                )
                                                            }
...
//                                    PrintSunmiUtils.cutPaperInner()
    ...
    private fun generateReceiptForU220(
        mPrinter: Printer, data: PrinterResponse.Data.KitchenReceiptPrinters, type: String
    ) {
        ...
        var tmps = "Open Order".toString().trim().toString().lowercase()
        ...
        if (kitchenSettingModel.showTeamMember) {
            ...
            mPrinter.addText(
                padLine(
                    "Employee:" + receiptModel?.order?.employee?.name, "", 33
                )
            )
        ...
        receiptModel?.order?.orderItems?.let { orderItems ->
            ...
            addOrdersForKitchenU220(
                mPrinter, printOrderItems, fontSizeH, fontSizeW, data.printerCategories.toCollection(arrayListOf())
            )
        }
        ...
        if (kitchenSettingModel.showCustomerAddress != false || kitchenSettingModel.showCustomerPhone != false || kitchenSettingModel.showCustomerName != false) {
            ...
            if (receiptModel?.order?.customer != null) {
             ...
             builder.addText(receiptModel?.order?.customer?.email)*/
                ...
                if (kitchenSettingModel.showCustomerAddress) {
                    if (receiptModel?.order?.orderType?.trim().toString().lowercase() == "Open Order".trim().toString()
                            .lowercase() && receiptModel?.order?.deliveryType?.trim().toString().lowercase() == "Pickup".trim().lowercase()
                    ) {
...
//            mPrinter.endTransaction()
        ...
        } catch (e: java.lang.Exception) {/* try {
                 mPrinter.disconnect()
             } catch (e: Exception) {
                 e.printStackTrace()
             }*/
    ...
    private fun generateKitchenReceipt(
        customerReceiptPrinters: PrinterResponse.Data.KitchenReceiptPrinters, type: String
    ) {
        var builder: Builder? = null
        try {
            LogUtil.logE(TAG, "KitchenPrinterName ${customerReceiptPrinters.name}")
            val pname = if (customerReceiptPrinters.name.substring(0, 6).toString().lowercase() == "TM-m30".lowercase()) {
                "TM-m30"
            } else {
                customerReceiptPrinters.name
            }
...
//                )
                ...
                receiptModel?.order?.orderItems?.let {
                    addOrdersForKitchen(
                        builder!!, it, fontSizeH, fontSizeW, customerReceiptPrinters.printerCategories.toCollection(arrayListOf())
                    )
                }
                        ...
                        if (kitchenSettingModel.showCustomerAddress) {
                            if (receiptModel?.order?.orderType?.trim().toString().lowercase() == "Open Order".trim().toString()
                                ...
                                if (receiptModel?.order?.customer?.addresses?.isNotEmpty() == true) {
                                    ...
                                    receiptModel?.order?.customer?.addresses?.filter { it.typeOfAddress == BILLING_ADDRESS }?.forEach {

                                        if (it.typeOfAddress.equals(
                                                BILLING_ADDRESS, ignoreCase = true
                                            )
                                        ) {
                                            builder!!.addText(
                                                it.fullAddress
...
//                )
                if (kitchenSettingModel.showTeamMember) {
                    ...
                    builder.addText(
                        padLine(
                            "Employee:" + receiptModel?.order?.employee?.name, "", if (kitchenSettingModel.fonts == LARGE) {
                                24
                            } else {
                                48
                            }
            ...
            try {
                PrinterClass.getPrinter()?.sendData(
                    builder, timeOut, status, battery
                )
...
//                printerDialog.dismiss()
    ...
    private fun generateKitchenReceiptSunmi(
        kitchenReceiptPrinters: PrinterResponse.Data.KitchenReceiptPrinters, type: String
    ) {
        try {

//            Log.d("KioskOpenOrderKitchenPrint", "${kitchenReceiptPrinters.name.toString()}")
            runBlocking {
                ...
                if (kitchenSettingModel.showTeamMember) {
                    ...
                    PrintSunmiUtils.employee(
                        padLine(
                            "Employee:" + receiptModel?.order?.employee?.name, "", if (kitchenSettingModel.fonts == LARGE) 23 else 48
                        ).toString()
                    )
                ...
                if (prefProvider.getValueboolean(OPEN_ORDER_UPDATE_FOR_PRINT, false) == true) {
                    receiptModel?.order?.orderItems?.let {
                        ...
                        addOrdersForKitchen(
                            if (printOrderItems.isNotEmpty()) printOrderItems else it, kitchenReceiptPrinters.printerCategories.toCollection(arrayListOf())
                        )
                    }
                    ...
                    receiptModel?.order?.orderItems?.let {
                        ...
                        addOrdersForKitchen(
                            it, kitchenReceiptPrinters.printerCategories.toCollection(arrayListOf())
                        )
                    }
                }
    ...
    private fun generateKitchenReceiptSunmiInner(
        ...
        if (!printingKitchen) {
            if (!kitchenReceiptPrinters.receiptPrintType.equals(
                    "KitchenAndCustomer", ignoreCase = true
                ) && !printingCustomer
            ) {
                printingKitchen = true
                try {
                    // PrintSunmiUtils.fontSizeInner(LARGE)
                    SunmiPrintHelper.getInstance().initPrinter()/*Added By Rahul */
                    try {
                        if (prefProvider.getValue(Constants.OPEN_ORDER_ITEMS_OLD, "").isNotEmpty()) {
                            if (isOrderUpdated == true || cartList!!.isEdited == true) {
                                PrintSunmiUtils.headerText("***** UPDATED *****")
                    ...
                    if (receiptModel?.order?.orderType.equals("Online Order", true) || receiptModel?.order?.orderType.equals(
                            "OnlineWebOrder", true
                    ...
                    if (kitchenSettingModel.showCustomerAddress != false || kitchenSettingModel.showCustomerPhone != false || kitchenSettingModel.showCustomerName != false) {
                        if (receiptModel?.order?.customer != null) {
                            ...
                            if (sunmiFrameworkVersion?.get(0)?.toInt()!! >= 3 && sunmiFrameworkVersion?.get(1)?.toInt()!! >= 3 && sunmiFrameworkVersion?.get(2)
                                    ?.toInt() != 39
                            ) {
                                PrintSunmiUtils.addHorizontalInnerNew()
                            } else {
...
//                                                }
    ...
    fun removeCustomer() {
        ...
        prefProvider.setValue(
            Constants.TOTAL_SERVICE_CHARGE_ACTUAL, "0.0"
        )
    ...
    private fun sunmiCloudPrintForGiftCard() = try {
        ...
        if (customerSettingModel.fonts == LARGE) {
            ...
            val str = padLine(
                ...
                )?.offline_id, "", if (customerSettingModel.fonts == LARGE) 23 else 48
            ).toString().trim()
        ...
        Log.d(
            TAG, "sunmiCloudPrintForGiftCard: giftCardAmount = ${giftCardAmount.toPrecision(2)}"
        )
        ...
        addOrderItems(
            giftCardList, false, customerSettingModel.fonts
        )

        SunmiPrinterApi.getInstance().lineWrap(1)

        val str5 = padLine(
            "Total Price", "$${giftCardAmount.toPrecision(2)}", if (customerSettingModel.fonts == LARGE) 23 else 48
        ).toString()
        PrintSunmiUtils.totalPrice(str5)
        ...
        if (giftCardReceiptModel?.gift_card?.payments?.get(giftCardReceiptModel?.gift_card?.payments?.size!! - 1)?.payment_type?.lowercase() == "Card".lowercase()) {
            ...
            val str12 = giftCardReceiptModel?.gift_card?.payments?.get(giftCardReceiptModel?.gift_card?.payments?.size!! - 1)?.card_name.toString()


            val str13 = giftCardReceiptModel?.gift_card?.payments?.get(giftCardReceiptModel?.gift_card?.payments?.size!! - 1)?.card_type.toString()


            val str14 = giftCardReceiptModel?.gift_card?.payments?.get(giftCardReceiptModel?.gift_card?.payments?.size!! - 1)?.card_number.toString()
            ...
            if (giftCardReceiptModel?.gift_card?.customer != null && !prefProvider.getValue(
                ...
                if (customerSettingModel.showCustomerPhone) {
                    ...
                    if (giftCardReceiptModel?.gift_card?.customer?.phones?.isNotEmpty() == true) {
                        ...
                        PrintSunmiUtils.customerPhone(
                            padLine(
                                number?.let { MethodUtils.formatPhoneNumber(it) }, "", if (customerSettingModel.fonts == LARGE) 23 else 48
                            ).toString()
                        )
...
//            SunmiPrinterApi.getInstance().lineWrap(1)
                ...
                if (customerSettingModel.fonts == LARGE) {
                    ...
                    val str = padLine(
                        "ReceiptID:" + receiptModel?.order?.offlineId?.trim(), "", if (customerSettingModel.fonts == LARGE) 23 else 48
                    ).toString().trim()
                ...
                receiptModel?.order?.orderItems?.let {
                    addOrderItems(
                        it, customerSettingModel.showModifiers, customerSettingModel.fonts
                    )
                }
                SunmiPrinterApi.getInstance().lineWrap(1)
                ...
                if (receiptModel?.order?.totalDiscount != null) {
                    ...
                    val str1 = padLine(
                        "Total Discount", if (receiptModel?.order?.totalDiscount == 0.0) {
//                        "-$" + MethodUtils.roundOffAmountString(receiptModel?.order?.totalDiscount!!)
                            "$" + MethodUtils.roundOffAmountString(receiptModel?.order?.totalDiscount!!)
                        } else {
                            "-$" + MethodUtils.roundOffAmountString(receiptModel?.order?.totalDiscount!!)
                ...
                val str2 = padLine(
                    "Sub Total", "$" + MethodUtils.roundOffAmountString(receiptModel?.order?.subTotal!!), if (customerSettingModel.fonts == LARGE) 23 else 48
                ).toString()
                PrintSunmiUtils.subTotal(str2)
                ...
                if (receiptModel?.order?.totalAmount != null) {
                    ...
                    if (paymentType == "Cash") {
                        ...
                        if (receiptModel?.order?.cash_discount_type?.lowercase() == "CashDiscount".lowercase()) {
                            ...
                            val str5 = padLine(
                                "Total Price", "$" + MethodUtils.roundOffAmountString(finalAmt), if (customerSettingModel.fonts == LARGE) 23 else 48
                            ).toString()
                            PrintSunmiUtils.totalPrice(str5)
                ...
                if (customerSettingModel.showTipSuggestion) {
                    ...
                    if (tipsList.isNotEmpty()) {
                        (if (receiptModel?.order?.totalDiscount != 0.0 && (receiptModel?.order?.totalAmount ?: 0.0) > (receiptModel?.order?.totalDiscount
                                ?: 0.0)
                        ) {/*(totalfamount)*/receiptModel?.order?.payments?.last()?.amount
                        } else {/*receiptModel?.order?.totalAmount!!*/receiptModel?.order?.payments?.last()?.amount
                        })?.let {
                            addTipsList(
                                tipsList, it, customerSettingModel.fonts
                            )
                        }
                        SunmiPrinterApi.getInstance().lineWrap(2)
...
//                            )
    ...
    private fun sunmiPrintInner(isAutoPrint: Boolean, fromllPrintButton: Boolean = false) {
        ...
        Log.d(TAG2, "Start printer sunmiPrintInner(isAutoPrint: Boolean) -> Here_2")
        EventBus.getDefault().post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragment.kt _sunmiPrintInner(isAutoPrint: Boolean) -> Here_2"))
        if (!printingCustomer) {
            printingCustomer = true
            GlobalScope.launch {
                try {
...
//                                    val str5 = padLine(
//                                        "Total Price",
//                                        "$" + receiptModel?.order?.payments?.last()?.amount?.plus(
//                                            receiptModel?.order?.payments?.last()?.tips!!
//                                        )?.let {
//                                            MethodUtils.roundOffAmountString(
//                                                it
...
//                    }
                    ...
                    val str5 = padLine(
                        "Total Price", "$" + receiptModel?.order?.payments?.last()?.amount?.plus(
                            receiptModel?.order?.payments?.last()?.tips!!
                        )?.let {
                            MethodUtils.roundOffAmountString(it)
                        }, if (customerSettingModel.fonts == LARGE) 23 else 48
...
//                    -----------------------------
                    ...
                    if (receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.paymentType?.lowercase() == "Card".lowercase()) {
                        ...
                        val strCardName = receiptModel?.order?.payments?.get(receiptModel?.order?.payments?.size!! - 1)?.cardName.toString()
                        ...
                        if (sunmiFrameworkVersion?.get(0)?.toInt()!! >= 3 && sunmiFrameworkVersion?.get(
                            ...
                            PrintSunmiUtils.cardDetailsInnerNew(
                                strCardName, strCardType, strCardNumber, customerSettingModel.fonts
                            )
                        } else {
                        ...
                        if (receiptModel?.order?.customer != null && !prefProvider.getValue(
                            ...
                            if (customerSettingModel.showCustomerPhone) {
                                ...
                                if (receiptModel?.order?.customer?.phones?.isNotEmpty() == true) {
                                    ...
                                    PrintSunmiUtils.normalText(
                                        padLine(
                                            phone?.let { MethodUtils.formatPhoneNumber(it) }, "", if (customerSettingModel.fonts == LARGE) 23 else 48
                                        ).toString()
                                    )
                            ...
                            if (customerSettingModel.showCustomerAddress) {
                                if (receiptModel?.order?.customer?.addresses?.isNotEmpty() == true) {
                                    ...
                                    PrintSunmiUtils.normalText(
                                        padLine(
                                            address, "", if (customerSettingModel.fonts == LARGE) 23 else 48
                                        ).toString()
                                    )
...
//                                            }
                    ...
                    if (customerSettingModel.showQrCode) {
                        SunmiPrintHelper.getInstance().lineWrap(1)
                        receiptModel?.order?.digital_receipt_url?.let {
                            PrintSunmiUtils.qrCodeInner(
                                it
                            )
                        }
                    ...
                    if (this@OrderCompleteFragment::kitchenReceiptPrinters.isInitialized && !fromllPrintButton) {
                        generateKitchenReceiptSunmiInnerAfterCustomer(
                            kitchenReceiptPrinters, ""
                        )
                        kitchenReceiptPrinted = true
            ...
            this.checkBluetoothPermissions(object : OnBluetoothPermissionGranted {
                override fun onPermissionsGranted() {
                    ...
                    try {
                        ...
                        EventBus.getDefault().post(
                            MessageEvent(
                                "${Constants.LINE_BREAK_TAB} OrderCompleteFragement.kt_landiInnerPrintForGiftCard() = ${
                                    Gson().toJson(printer)
                                }"
                            )
                        )
                        ...
                        EventBus.getDefault()
                            .post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragement.kt_landiInnerPrintForGiftCard()_Catch = ${ex.printStackTrace()}"))
...
//                                    lineBreak()
                                    ...
                                    if (customerSettingModel.showOrderType) {
                                        giftCardReceiptModel?.gift_card?.order_type_name?.trim()?.let {
//                                        outputStream.write(LPrint.FONT_SIZE_5X)
//
//                                        outputStream.write(it.toByteArray())
                                    ...
                                    var giftCardAmount = 0.00
                                    if (giftCardReceiptModel?.gift_card?.payments != null && giftCardReceiptModel?.gift_card?.payments?.isNotEmpty()!!) {
                                        giftCardAmount = giftCardReceiptModel?.gift_card?.payments?.get(
                                    ...
                                    Log.d(
                                        TAG, "landiInnerPrintForGiftCard:  giftCardAmount = ${
                                    ...
                                    addOrderItemsInnerNewLandi(
                                        giftCardList, false, customerSettingModel.fonts, lprint = LPrint
                                    )
                                    ...
                                    val str5 = padLine(
                                        "Sub Total", "$${giftCardAmount.toPrecision(2)}", if (customerSettingModel.fonts == LARGE) 23 else 48
                                    ).toString()
                                    ...
                                    if (giftCardReceiptModel?.gift_card?.payments?.isNotEmpty() == true && giftCardReceiptModel?.gift_card?.payments?.get(
                                        ...
                                        val str8 = padLine(
                                            Constants.SURCHARGE_TEXT, "$" + surcharge?.let {
                                                MethodUtils.roundOffAmountString(
                                                    it
                                                )
                                            }, if (customerSettingModel.fonts == LARGE) 23 else 48
                                    ...
                                    if (customerSettingModel.showRefundAmount && giftCardReceiptModel?.gift_card?.payments?.get(
                                            giftCardReceiptModel?.gift_card?.payments?.size!! - 1
                                        )?.payment_type?.lowercase() == "Cash".lowercase()
                                    ) {

                                        val str7 = padLine(
                                            "Change Amount", "$" + MethodUtils.roundOffAmountString(
                                                changeAmtGlobal
                                            ), if (customerSettingModel.fonts == LARGE) 23 else 48
                                        ).toString()
                                    ...
                                    val str10 = padLine(
                                        "Transaction ID", "" + giftCardReceiptModel?.gift_card?.payments?.size?.minus(
                                            1
                                        )?.let {
                                    ...
                                    val str11 = padLine(
                                        "Transaction Type", giftCardReceiptModel?.gift_card?.payments?.get(
                                            giftCardReceiptModel?.gift_card?.payments?.size!! - 1
                                        )?.payment_type, if (customerSettingModel.fonts == LARGE) 23 else 48
                                    ).toString()
                                    ...
                                    if (giftCardReceiptModel?.gift_card?.payments?.get(
                                        ...
                                        val str12 = giftCardReceiptModel?.gift_card?.payments?.get(
                                            giftCardReceiptModel?.gift_card?.payments?.size!! - 1
                                        )?.card_name.toString()


                                        val str13 = giftCardReceiptModel?.gift_card?.payments?.get(
                                            giftCardReceiptModel?.gift_card?.payments?.size!! - 1
                                        )?.card_type.toString()


                                        val str14 = giftCardReceiptModel?.gift_card?.payments?.get(
                                            giftCardReceiptModel?.gift_card?.payments?.size!! - 1
                                        )?.card_number.toString()

                                        cardDetailsInner(
                                            str12, str13, str14, customerSettingModel.fonts
                                        )
                                        lineBreak()
                                    }
    ...
    private fun sunmiInnerPrintForGiftCard() {
        ...
        try {
            ...
            if (customerSettingModel.showOrderType) {
                giftCardReceiptModel?.gift_card?.order_type_name?.trim()?.let { PrintSunmiUtils.headerText(it) }
                SunmiPrintHelper.getInstance().lineWrap(1)
            }
            ...
            Log.d(
                TAG, "sunmiInnerPrintForGiftCard:  giftCardAmount = ${giftCardAmount.toPrecision(2)}"
            )
            ...
            if (giftCardReceiptModel?.gift_card?.payments?.isNotEmpty() == true && giftCardReceiptModel?.gift_card?.payments?.get(
                ...
                val surcharge = giftCardReceiptModel?.gift_card?.payments?.last()?.cash_discount_or_surcharge
                val str8 = padLine(
                    Constants.SURCHARGE_TEXT, "$" + surcharge?.let { MethodUtils.roundOffAmountString(it) }, if (customerSettingModel.fonts == LARGE) 23 else 48
                ).toString()
            ...
            val totalPrice = giftCardAmount + giftCardReceiptModel?.gift_card?.payments?.last()?.cash_discount_or_surcharge!!
            val str20 = padLine(
                "Total Price", "$" + MethodUtils.roundOffAmountString(totalPrice), if (customerSettingModel.fonts == LARGE) 23 else 48
            ).toString()
            if (sunmiFrameworkVersion?.get(0)?.toInt()!! >= 3 && sunmiFrameworkVersion?.get(1)?.toInt()!! >= 3 && sunmiFrameworkVersion?.get(2)
                    ?.toInt() != 39
            ) {
...
//                                    }
    ...
    private fun printBusinessLogo() {
        val decodedString: ByteArray = Base64.decode(
            prefProvider.getValue(VENUE_LOGO, ""), Base64.NO_CLOSE
        )
        val bitmap: Bitmap = BitmapFactory.decodeByteArray(decodedString, 0, decodedString.size)
    ...
    private fun setService1(isAutoPrint: Boolean, fromllPrintButton: Boolean = false) {
        Log.d(TAG2, "Start printer _setService1(isAutoPrint) -> Here")
        EventBus.getDefault().post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragment.kt _setService1(isAutoPrint) -> Here"))
        ...
        if (SunmiPrintHelper.getInstance().sunmiPrinter == SunmiPrintHelper.FoundSunmiPrinter) {
            Log.d(TAG2, "Start printer _setService1(isAutoPrint) -> Here_2")
            EventBus.getDefault().post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragment.kt _setService1(isAutoPrint) -> Here_2"))
            ...
            Log.d(TAG2, "Start printer _setService1(isAutoPrint) -> Here_3")
            EventBus.getDefault().post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragment.kt _setService1(isAutoPrint) -> Here_3"))

            Handler(Looper.getMainLooper()).postDelayed({
                setService1(isAutoPrint)
            }, 2000)
        } else if (SunmiPrintHelper.getInstance().sunmiPrinter == SunmiPrintHelper.LostSunmiPrinter) {
            Log.d(TAG2, "Start printer _setService1(isAutoPrint) -> Here_4")
            EventBus.getDefault().post(MessageEvent("${Constants.LINE_BREAK_TAB} OrderCompleteFragment.kt _setService1(isAutoPrint) -> Here_4"))
...
//                Log.d("KioskOpenOrderKitchenPrint", "isBlueToothPrinter-${kitchenReceiptPrinters.name.toString()}")
            ...
            Handler(Looper.getMainLooper()).postDelayed({
//                Log.d("KioskOpenOrderKitchenPrint", "CheckSunmiPrinter-${kitchenReceiptPrinters.name.toString()}")
    ...
    @Throws(
        NoSuchAlgorithmException::class, InvalidKeySpecException::class, InvalidKeyException::class, SignatureException::class
    )
    fun sign(
        body: String, appId: String, timestamp: String, nonce: String, rsaPrivateKey: String
    ): String {
        val content = body + appId + timestamp + nonce
        val keyBytes: ByteArray = java.util.Base64.getDecoder().decode(rsaPrivateKey.replace("(\\s)|(--.*--)".toRegex(), ""))
    ...
    fun pushContent(
        trade_no: String?, sn: String?, count: Int, order_type: Int, media_text: String?, cycle: Int
    ): String? {
        val body = java.lang.StringBuilder()
        body.append("{")
    ...
    fun setAbsolutePrintPosition(n: Int) {
        if (n >= 0 && n <= 65535) orderContent.append(
            "1b24" + String.format(
                "%02x%02x", (n and 0xff), ((n shr 8) and 0xff)
            )
        )
    }
