...
class IssueRefundDialog : DialogFragment(), TextWatcher {
    ...
    fun calculationOfItems() {
        ...
        selectedLoyaltyPointDivided =
            MethodUtils.roundOffAmountDouble(selectedLoyaltyPointDivided)
    ...
    private fun startRefundProcess() {
        if (isItem) {
            if (TextUtils.isEmpty(binding.edtAmount.text.toString())) {
                ...
                val refundedAmount =
                    paymentOrderDetailsResponse.data.order.refund_detail.refunded_amount
                ...
                refundData = RefundRequestModel().apply {
                    paymentRefund = RefundRequestModel.PaymentRefund().apply {
                        ...
                        terminalId = paymentOrderDetailsResponse.data.terminal_id
                        taxRefunded =
                            if (screenTotalAmount.toDouble() > (refundAmount + refundedAmount)) 0.0 else paymentOrderDetailsResponse.data.tax_amount
                        tipsRefunded =
            ...
            if (!checkIfSelectedItems()) {
                ...
                refundData = RefundRequestModel().apply {
                    paymentRefund = RefundRequestModel.PaymentRefund().apply {
                        refundItemListAdapter.selectedItemList().forEach { it ->
                            ...
                            terminalId = paymentOrderDetailsResponse.data.terminal_id
                            taxRefunded =
                                if (screenTotalAmount.toDouble() > (totalCheckedItemPrice + refundedAmount)) 0.0 else paymentOrderDetailsResponse.data.tax_amount
                            tipsRefunded =
                                if (paymentOrderDetailsResponse.data.payment_type == "Cash") 0.0 else paymentOrderDetailsResponse.data.tips
                            serviceChargeRefunded =
                ...
                if (String.format("%.2f", (totalCheckedItemPrice + refundedAmount))
                    ...
                    if (String.format("%.2f", (totalCheckedItemPrice + refundedAmount))
                            .toDouble() > String.format("%.2f", screenTotalAmount.toDouble())
                            .toDouble()
                    ) {
